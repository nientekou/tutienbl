...
const TravelerService_1 = require("./services/TravelerService");
...
async function runTests() {
    ...
    database_1.default.prepare('DELETE FROM users WHERE discord_id IN (?, ?, ?)').run(testUser, targetUser, thirdUser);
    database_1.default.prepare('DELETE FROM user_caves WHERE user_id IN (?, ?, ?)').run(testUser, targetUser, thirdUser);
    database_1.default.prepare('DELETE FROM inventories WHERE user_id IN (?, ?, ?)').run(testUser, targetUser, thirdUser);
    database_1.default.prepare('DELETE FROM mounts WHERE user_id IN (?, ?, ?)').run(testUser, targetUser, thirdUser);
    database_1.default.prepare('DELETE FROM traveler_events').run();
    database_1.default.prepare('DELETE FROM user_encounters WHERE user_id IN (?, ?, ?)').run(testUser, targetUser, thirdUser);
  ...
  `).run();
    ...
    database_1.default.prepare('UPDATE users SET exp_needed = 10000 WHERE discord_id IN (?, ?, ?)').run(testUser, targetUser, thirdUser);
    UserRepository_1.userRepository.cache.delete(testUser);
    UserRepository_1.userRepository.cache.delete(targetUser);
    UserRepository_1.userRepository.cache.delete(thirdUser);
    ...
    InventoryRepository_1.inventoryRepository.addItem(testUser, 'thung_bat_thu', 5);
    const captureRes = MountService_1.mountService.captureMount(testUser, 'thung_bat_thu');
    // Let's get the mount we just tamed/captured
    const mounts = MountService_1.mountService.getMounts(testUser);
    if (mounts.length === 0) {
    ...
    const activeMount = MountService_1.mountService.getMounts(testUser)[0];
    console.log(`Đã chuẩn bị tọa kỵ: ${activeMount.name} (ID: ${activeMount.id}, Level: ${activeMount.level})`);
    // Add 5 materials
    InventoryRepository_1.inventoryRepository.addItem(testUser, 'lenh_bai', 5);
    let invItem = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND item_id = ?').get(testUser, 'lenh_bai');
    console.log(`Số lượng nguyên liệu trước khi nuôi: ${invItem.quantity}`);
    const feedRes = MountService_1.mountService.feedMount(testUser, activeMount.id, 'lenh_bai');
    console.log(`Kết quả feedMount: ${feedRes.message}`);
    invItem = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND item_id = ?').get(testUser, 'lenh_bai');
    ...
    database_1.default.prepare('UPDATE users SET updated_at = ? WHERE discord_id = ?').run(twoHoursAgo, testUser);
    UserRepository_1.userRepository.cache.delete(testUser);
    let userBefore = UserRepository_1.userRepository.get(testUser);
    ...
    const springRes = CaveService_1.caveService.collectSpring(testUser);
    console.log(`Kết quả ngâm linh tuyền: ${springRes.message}`);
    let userAfterSpring = UserRepository_1.userRepository.get(testUser);
    ...
    database_1.default.prepare('UPDATE users SET updated_at = ? WHERE discord_id = ?').run(twoHoursAgo, testUser);
    UserRepository_1.userRepository.cache.delete(testUser);
    userBefore = UserRepository_1.userRepository.get(testUser);
    console.log(`Trước khi nuốt đan dược: Tu Vi: ${userBefore.tu_vi}`);
    InventoryRepository_1.inventoryRepository.addItem(testUser, 'pill_tu_vi_low', 1);
    const pillInInv = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND item_id = ?').get(testUser, 'pill_tu_vi_low');
    console.log('-> Nuốt đan dược...');
    const usePillRes = InventoryService_1.inventoryService.useItem(testUser, pillInInv.id);
    console.log(`Kết quả nuốt đan dược: ${usePillRes.message}`);
    const userAfterPill = UserRepository_1.userRepository.get(testUser);
  ...
  `).run(nowSec, expiresAt, JSON.stringify(mockInventory));
    const eventId = insertRes.lastInsertRowid;
    console.log(`Đã tạo event Lữ Khách Thần Bí ID: ${eventId}`);
    // Test buyItem
    console.log('-> Thực hiện mua vật phẩm...');
    const buyRes = TravelerService_1.travelerService.buyItem(testUser, eventId, 'tang_bao_do', 1);
    console.log(`Kết quả mua hàng: ${buyRes.message}`);
    const boughtItem = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND item_id = ?').get(testUser, 'tang_bao_do');
    ...
    const robRes = TravelerService_1.travelerService.challengeTraveler(testUser, eventId);
    ...
    InventoryRepository_1.inventoryRepository.addItem(testUser, 'lucky_chest', 1);
    const chestItem = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND item_id = ?').get(testUser, 'lucky_chest');
    console.log(`Đã thêm rương vào túi đồ, ID: #${chestItem.id}`);
    const openRes = InventoryService_1.inventoryService.useItem(testUser, chestItem.id);
    ...
    database_1.default.prepare('UPDATE users SET tu_vi = 0 WHERE discord_id = ?').run(testUser);
    UserRepository_1.userRepository.cache.delete(testUser);
    const encounterUserBefore = UserRepository_1.userRepository.get(testUser);
    ...
    const encounterRes = EncounterService_1.encounterService.resolveEncounter(testUser, 'sy_cao_nhan', 'sy_cn_nhan');
    console.log(`Kết quả Kỳ Ngộ: ${encounterRes.message}`);
    const encounterUserAfter = UserRepository_1.userRepository.get(testUser);
  ...
  `).run(testUser, customStats, Math.floor(Date.now() / 1000));
    ...
    database_1.default.prepare("INSERT OR IGNORE INTO sects (id, name, master_id, level, resources, created_at) VALUES (999, 'Thần Hoàn Sect', ?, 5, 0, 0)").run(testUser);
    database_1.default.prepare("INSERT OR IGNORE INTO sects (id, name, master_id, level, resources, created_at) VALUES (998, 'Vạn Tượng Sect', ?, 5, 0, 0)").run(targetUser);
    database_1.default.prepare("INSERT OR IGNORE INTO sects (id, name, master_id, level, resources, created_at) VALUES (997, 'Thái Cực Sect', ?, 5, 0, 0)").run(thirdUser);
    database_1.default.prepare("UPDATE users SET sect_id = 999, sect_role = 'leader' WHERE discord_id = ?").run(testUser);
    database_1.default.prepare("UPDATE users SET sect_id = 998, sect_role = 'leader' WHERE discord_id = ?").run(targetUser);
    database_1.default.prepare("UPDATE users SET sect_id = 997, sect_role = 'leader' WHERE discord_id = ?").run(thirdUser);
    UserRepository_1.userRepository.cache.delete(testUser);
    UserRepository_1.userRepository.cache.delete(targetUser);
    UserRepository_1.userRepository.cache.delete(thirdUser);
    // Join battle
    const joinRes = SectWarService_1.sectWarService.joinBattle(testUser);
    console.log(`Kết quả gia nhập Sect War: ${joinRes.message}`);
    // Get active battles
    let battles = SectWarService_1.sectWarService.getActiveBattlesForSect(999);
    if (battles.length > 0) {
        ...
        database_1.default.prepare("UPDATE sect_war_battles SET sect_ids = ? WHERE id = ?")
            .run(JSON.stringify([999, 998, 997]), battles[0].id);
        // Test attack
        const attackRes = SectWarService_1.sectWarService.attack(testUser, targetUser);
