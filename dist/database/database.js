...
function createDB() {
    const db = new better_sqlite3_1.default(config_1.config.dbPath, { verbose: process.env.NODE_ENV === 'development' ? console.log : undefined });
    ...
    try {
        ...
        db.close();
        const db2 = new better_sqlite3_1.default(config_1.config.dbPath);
...
function initDatabase() {
  ...
  `);
    ...
    try {
        const invCols = db.prepare("PRAGMA table_info(inventories)").all();
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE pets ADD COLUMN mood INTEGER DEFAULT 50`);
    }
    catch (e) { } // 0-100 happiness
    try {
        db.exec(`ALTER TABLE pets ADD COLUMN personality TEXT DEFAULT 'friendly'`);
    }
    catch (e) { } // friendly/lazy/aggressive
    try {
        db.exec(`ALTER TABLE pets ADD COLUMN element TEXT DEFAULT 'none'`);
    }
    catch (e) { }
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE soul_weapons ADD COLUMN element TEXT DEFAULT 'Vo'`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE soul_weapons ADD COLUMN evolution_stage INTEGER DEFAULT 1`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE user_bloodlines ADD COLUMN evolution_stage INTEGER DEFAULT 1`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE soul_weapons ADD COLUMN skills_json TEXT DEFAULT '[]'`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE soul_weapons ADD COLUMN awakening_level INTEGER DEFAULT 0`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE soul_weapons ADD COLUMN skin_id TEXT DEFAULT NULL`);
    }
    catch (e) { }
  ...
  `);
    ...
    const hasBossPoints = db.prepare("PRAGMA table_info(users)").all().some((c) => c.name === 'boss_points');
    if (!hasBossPoints) {
        db.exec(`ALTER TABLE users ADD COLUMN boss_points INTEGER DEFAULT 0`);
    }
    // HP hiện tại của người chơi (cho World Boss phản phệ, etc.)
    const hasHpCol = db.prepare("PRAGMA table_info(users)").all().some((c) => c.name === 'hp');
    if (!hasHpCol) {
  ...
  `);
    ...
    const hasContribSeason = db.prepare("PRAGMA table_info(world_boss_contributions)").all().some((c) => c.name === 'season_id');
    if (!hasContribSeason) {
  ...
  `);
    ...
    const guildConfigCols = db.prepare("PRAGMA table_info(guild_configs)").all();
  ...
  `);
    ...
    const sectColumns = db.prepare("PRAGMA table_info(sects)").all();
    ...
    const invColumns = db.prepare("PRAGMA table_info(inventories)").all();
    ...
    const plotColumns = db.prepare("PRAGMA table_info(farming_plots)").all();
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE daily_quests ADD COLUMN is_elite INTEGER DEFAULT 0`);
    }
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE users ADD COLUMN bounty_tokens INTEGER DEFAULT 0`);
    }
    catch (e) { /* column exists */ }
    // P5-01: Reincarnation V2 — Dao Tam + Tokens
    try {
        db.exec(`ALTER TABLE users ADD COLUMN reincarnation_tokens INTEGER DEFAULT 0`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE users ADD COLUMN dao_tam TEXT DEFAULT NULL`);
    }
    catch (e) { } // JSON: { element: 'Hoa', level: 1, buffs: {...}, nerfs: {...} }
  ...
  `);
    ...
    const marketListingColumns = db.prepare("PRAGMA table_info(market_listings)").all();
  ...
  `);
    ...
    const bloodlineCheck = db.prepare('SELECT count(*) as count FROM bloodlines').get();
  ...
  `);
    ...
    try {
        const caveCols = db.prepare("PRAGMA table_info(user_caves)").all();
  ...
  `);
    try {
        const mountCols = db.prepare("PRAGMA table_info(mounts)").all();
    ...
    try {
        const arenaCols = db.prepare("PRAGMA table_info(arena_profiles)").all();
  ...
  `);
    ...
    try {
        const brokenAchievements = db.prepare("SELECT ua.user_id, ua.achievement_id, a.reward_exp, a.reward_coins FROM user_achievements ua JOIN achievements a ON ua.achievement_id = a.id WHERE ua.is_completed = 1 AND ua.completed_at IS NULL").all();
        if (brokenAchievements.length > 0) {
            const now = Math.floor(Date.now() / 1000);
            const fixStmt = db.prepare('UPDATE user_achievements SET completed_at = ? WHERE user_id = ? AND achievement_id = ?');
            ...
            for (const row of brokenAchievements) {
                ...
                if (row.reward_exp > 0)
                    awardExpStmt.run(row.reward_exp, row.user_id);
                if (row.reward_coins > 0)
                    awardCoinsStmt.run(row.reward_coins, row.user_id);
            }
  ...
  `);
    const transaction = db.transaction((items) => {
        for (const item of items) {
            stmt.run(item.id, item.name, item.type, item.rarity, item.description, item.stats, item.value_ha_pham, item.usable, item.equipable);
        }
    });
...
function seedSpiritSkills() {
    const existingCount = db.prepare("SELECT COUNT(*) as c FROM spirit_skills").get();
...
function seedHeartLaws() {
    const existingCount = db.prepare("SELECT COUNT(*) as c FROM heart_laws").get();
    if (existingCount.c > 0)
        return;
    const laws = [
