...
try {
    ...
    database_2.default.prepare('DELETE FROM user_achievements WHERE user_id = ?').run(testDiscordId);
    database_2.default.prepare('DELETE FROM user_titles WHERE user_id = ?').run(testDiscordId);
    database_2.default.prepare('DELETE FROM audit_logs WHERE user_id = ?').run(testDiscordId);
    ...
    const userBefore = UserRepository_1.userRepository.get(testDiscordId);
    ...
    UserRepository_1.userRepository.update(testDiscordId, { ngotinh: 100 });
    ...
    UserRepository_1.userRepository.update(testDiscordId, { y_canh: JSON.stringify(intents) });
    ...
    while (!success && attempts < 20) {
        attempts++;
        UserRepository_1.userRepository.update(testDiscordId, { ngotinh: 100 }); // Tiếp tế ngộ tính
        const res = CultivationService_1.cultivationService.awakenYCanh(testDiscordId);
        if (res.success && res.yCanhName === 'Kiếm Ý' && res.newLevel === 10) {
            ...
            const ua = database_2.default.prepare('SELECT * FROM user_achievements WHERE user_id = ? AND achievement_id = ?').get(testDiscordId, 'tl_15');
            ...
            const userAfter = UserRepository_1.userRepository.get(testDiscordId);
            ...
            // Xác minh danh hiệu
            const titleExists = database_2.default.prepare('SELECT * FROM user_titles WHERE user_id = ? AND title = ?').get(testDiscordId, 'Kiếm Ý Đại Thành');
            if (titleExists) {
    ...
    database_2.default.prepare('DELETE FROM user_achievements WHERE user_id = ? AND achievement_id = ?').run(testDiscordId, 'tl_16');
  ...
  `).run(testDiscordId);
    ...
    UserRepository_1.userRepository.update(testDiscordId, { y_canh: JSON.stringify(intents2), coin_ha_pham: 0 }); // Reset coin về 0
    console.log('Đã giả lập bản ghi tl_16 bị lỗi (completed_at = null, coin = 0). Gọi setProgress để kiểm tra tự sửa lỗi...');
    const healRes = AchievementService_1.achievementService.setProgress(testDiscordId, 'tl_16', 10);
    ...
    const uaHealed = database_2.default.prepare('SELECT * FROM user_achievements WHERE user_id = ? AND achievement_id = ?').get(testDiscordId, 'tl_16');
    ...
    const userAfterHeal = UserRepository_1.userRepository.get(testDiscordId);
    ...
    database_2.default.prepare('DELETE FROM audit_logs WHERE user_id = ? AND action = ?').run(testDiscordId, 'practice');
    database_2.default.prepare('DELETE FROM user_achievements WHERE user_id = ? AND achievement_id = ?').run(testDiscordId, 'tl_9');
    console.log('Giả lập chạy thiền định 10 lần...');
    for (let i = 0; i < 9; i++) {
        CultivationService_1.cultivationService.practice(testDiscordId);
    }
    // Lần thứ 10 sẽ đạt mốc thành tựu tl_9
    const practice10 = CultivationService_1.cultivationService.practice(testDiscordId);
