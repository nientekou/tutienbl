...
exports.performHunt = performHunt;
exports.checkPetAchievements = checkPetAchievements;
exports.getSanYeuThuComponents = getSanYeuThuComponents;
exports.getSanYeuThuEmbed = getSanYeuThuEmbed;
...
function performHunt(userId) {
    ...
    let rolledEncounter = null;
    // Kiểm tra Thể Lực
    // Mount stamina save bonus
    const activeMount = database_1.default.prepare('SELECT stamina_save FROM mounts WHERE user_id = ? AND is_active = 1').get(userId);
    ...
    const petRaw = database_1.default.prepare('SELECT name, base_atk, level, skills, mutations FROM pets WHERE user_id = ? AND is_deployed = 1').get(userId);
    ...
    if (petRaw) {
        ...
        let skills = [];
        try {
            skills = JSON.parse(petRaw.skills || '[]');
        }
        catch (e) { }
        activePet = {
            name: petRaw.name,
    ...
    const combatResult = CombatEngine_1.CombatEngine.run(playerCombatant, enemyCombatant, activePet ? { name: activePet.name, atk: activePet.atk, skills: activePet.skills } : null, 30);
    ...
    if (isWin) {
        ...
        if (user.sect_id) {
            ...
            if (sect) {
                try {
                    ...
                    if (b.tangkinhcac)
                        sectBonusPercent += b.tangkinhcac * 2; // +2% mỗi cấp
                }
                catch (e) { }
            }
        }
        ...
        database_1.default.transaction(() => {
            ...
            const activePet = database_1.default.prepare('SELECT * FROM pets WHERE user_id = ? AND is_deployed = 1').get(userId);
            if (activePet) {
                ...
                let newPetSkills = [];
                try {
                    newPetSkills = JSON.parse(activePet.skills || '[]');
                }
                catch (e) { }
                ...
                while (newPetExp >= expNeeded) {
                    ...
                    if (newPetLevel === 30 && newPetSkills.length < 3) {
                        const lockedKeys = Object.keys(sungthu_1.PET_SKILLS).filter(k => !newPetSkills.includes(k));
                        if (lockedKeys.length > 0) {
                            const newSkill = lockedKeys[Math.floor(Math.random() * lockedKeys.length)];
                            newPetSkills.push(newSkill);
                            const skDetails = sungthu_1.PET_SKILLS[newSkill];
        ...
        if (rollCapture < monster.captureRate) {
            ...
            let petSkills = '[]';
            if (monster.templateId === 'pet_qilin')
                petSkills = JSON.stringify(['qilin_heal']);
            else if (monster.templateId === 'pet_kun_pen')
                petSkills = JSON.stringify(['kunpen_hp']);
            else if (monster.templateId === 'pet_taotie')
                petSkills = JSON.stringify(['taotie_def']);
            else if (monster.templateId === 'pet_black_dragon')
                petSkills = JSON.stringify(['dragon_berserk']);
            else if (monster.templateId === 'pet_fire_phoenix')
                petSkills = JSON.stringify(['phoenix_rebirth']);
            else if (monster.templateId === 'pet_sky_bird')
                petSkills = JSON.stringify(['sky_agile']);
            else if (monster.templateId === 'pet_nine_tail')
                petSkills = JSON.stringify(['nine_charm']);
            else if (monster.templateId === 'kylan')
                petSkills = JSON.stringify(['qilin_fortune']);
            else if (monster.templateId === 'phuonghoang')
                petSkills = JSON.stringify(['reborn_flame']);
            else if (monster.templateId === 'tyhuu')
                petSkills = JSON.stringify(['gold_blessing']);
            database_1.default.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0, 0, ?, ?, ?)
      `).run(userId, monster.name.split(' ')[0], monster.templateId, monster.rarity, monster.petBaseHp, monster.petBaseAtk, monster.petBaseDef, gender, now, petSkills);
        ...
        const content = [
            (0, v2Components_1.header)(`🌲 CHIẾN THẮNG DÃ NGOẠI - ĐẢ THẢO TIỂU ĐIỀN`, 'Thảo phạt thành công quái thú hoang dã.'),
            (0, v2Components_1.separator)(),
            (0, v2Components_1.body)(`Đạo hữu đã thảo phạt thành công **${monster.name}** sau **${combatResult.rounds}** hiệp đấu!\n\n` +
        ...
        if (isCaptured) {
            ...
            content.push((0, v2Components_1.body)(`**🎉 CƠ DUYÊN THU PHỤC LINH THÚ 🎉**\n` +
            ...
            content.push((0, v2Components_1.separator)());
            content.push((0, v2Components_1.body)(`*Mách nhỏ: Linh thú sau khi bị đánh bại đã chấn kinh chạy mất dạng.*`));
        }
        // Kiểm tra Kỳ Ngộ (10%)
        rolledEncounter = EncounterService_1.encounterService.rollEncounter('sanyeuthu');
        if (rolledEncounter) {
            content.push((0, v2Components_1.separator)());
            content.push((0, v2Components_1.body)(`**🌟 Kỳ Ngộ: ${rolledEncounter.title}**\n` +
                `${rolledEncounter.description}\n\n` +
                `**Lựa chọn:**\n${rolledEncounter.choices.map((c, i) => `**${i + 1}.** ${c.text} (${Math.round(c.successRate * 100)}% thành công)`).join('\n')}`));
        }
        // Cơ hội bắt Tọa Kỵ (5%)
        if (Math.random() < 0.05) {
      ...
      `).run(userId, tmpl.name, tmpl.id, tmpl.rarity, tmpl.speed, tmpl.stamina, now);
            content.push((0, v2Components_1.separator)());
            content.push((0, v2Components_1.body)(`**🐎 CƠ DUYÊN THU PHỤC TỌA KKi!**\nĐạo hữu đã thu phục được **${tmpl.name}** [${tmpl.rarity.toUpperCase()}]! Dùng \`/toaky danhsach\` để xem chi tiết.`));
        }
        embed = (0, v2Components_1.container)(v2Components_1.V2_COLORS.success, content);
    }
    else {
        database_1.default.transaction(() => {
            UserRepository_1.userRepository.update(userId, { stamina: user.stamina - staminaCost });
        })();
        embed = (0, v2Components_1.container)(v2Components_1.V2_COLORS.danger, [
            (0, v2Components_1.header)(`💀 THẤT BẠI DÃ NGOẠI`, 'Chiến lực bất túc, đành phải bại lui.'),
            (0, v2Components_1.separator)(),
            (0, v2Components_1.body)(`Đạo hữu cự địch bất thành, kiệt sức tháo lui trước sức mạnh hoang dại của **${monster.name}** sau **${combatResult.rounds}** hiệp đấu!\n\n` +
                `⚡ **Thể lực hao tổn:** **-${staminaCost}** Thể Lực *(Còn lại: ${user.stamina - staminaCost}/500)*\n` +
                `${(0, constants_1.getProgressBar)(user.stamina - staminaCost, 500, 10)}\n\n` +
    ...
    try {
        ...
        database_1.default.prepare("INSERT INTO audit_logs (user_id, action, details, created_at) VALUES (?, 'hunt', ?, ?)").run(userId, JSON.stringify({ monsterName: monster.name, win: isWin }), nowLog);
        // Cập nhật tiến trình thành tựu
        const totalHunts = database_1.default.prepare("SELECT COUNT(*) as c FROM audit_logs WHERE user_id = ? AND action = 'hunt'").get(userId);
        AchievementService_1.achievementService.setProgress(userId, 'cd_12', totalHunts.c);
        AchievementService_1.achievementService.setProgress(userId, 'cd_13', totalHunts.c);
    }
    catch (e) {
        console.error('[sanyeuthu achievement error]', e);
    }
    // Thêm năng lượng cho linh mạch Chiến Đấu
    LeylineService_1.leylineService.addEnergy(userId, 'chiendau', 10);
 ...
 */
function checkPetAchievements(userId, newPetRarity) {
    const pets = database_1.default.prepare('SELECT id, rarity, level FROM pets WHERE user_id = ?').all(userId);
    const totalPets = pets.length;
    // Thành tựu số lượng sủng thú
    AchievementService_1.achievementService.setProgress(userId, 'st_1', totalPets);
    AchievementService_1.achievementService.setProgress(userId, 'st_2', totalPets);
    AchievementService_1.achievementService.setProgress(userId, 'st_3', totalPets);
    AchievementService_1.achievementService.setProgress(userId, 'st_4', totalPets);
    ...
    if (newPetRarity) {
        ...
        if (hasRare)
            AchievementService_1.achievementService.updateProgress(userId, 'st_5', 1);
        if (hasEpic)
            AchievementService_1.achievementService.updateProgress(userId, 'st_6', 1);
        if (hasLegendary)
            AchievementService_1.achievementService.updateProgress(userId, 'st_7', 1);
    }
 ...
 */
function getSanYeuThuComponents(userId) {
    const user = UserRepository_1.userRepository.get(userId);
    ...
    return [
        new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            ...
            .setStyle(enoughStamina ? discord_js_1.ButtonStyle.Success : discord_js_1.ButtonStyle.Secondary)
...
function getSanYeuThuEmbed(userId) {
    const user = UserRepository_1.userRepository.get(userId);
    if (!user) {
        return (0, v2Components_1.container)(v2Components_1.V2_COLORS.danger, [(0, v2Components_1.header)('❌ Lỗi'), (0, v2Components_1.body)('Không tìm thấy nhân vật.')]);
    }
    let tier = 'Luyện Khí';
    let rateText = '30% (Thường)';
    let monstersText = `• **U Linh Thử 🐭** (HP: 80 │ Công: 12 │ Thủ: 5)\n` +
    ...
    const hpBar = (0, constants_1.getProgressBar)(user.stamina, 500, 10);
    return (0, v2Components_1.container)(v2Components_1.V2_COLORS.success, [
        (0, v2Components_1.header)(`🐺 SĂN YÊU THÚ DÃ NGOẠI - ${user.name}`, 'Ngoài hoang dã bao la, yêu thú tứ phương hội tụ. Đạo hữu có thể phiêu du đả thảo tiểu điền để tìm cơ duyên.'),
        (0, v2Components_1.separator)(),
        (0, v2Components_1.body)(`🔋 **Thể Lực Hiện Tại:** **${user.stamina}/500** Thể Lực\n${hpBar}\n\n` +
            `🏔️ **Cảnh Giới Hiện Tại:** **${tier}** (Cấp ${user.level})\n` +
            `⚡ **Thể Lực Tiêu Hao:** **15** mỗi lần săn`),
        (0, v2Components_1.separator)(),
        (0, v2Components_1.body)(`🎯 **Linh Thú Xuất Hiện Tại Khu Vực:**\n${monstersText}\n\n✨ **Tỷ lệ thu phục thành công:** **${rateText}**`),
        (0, v2Components_1.separator)(),
        (0, v2Components_1.body)(`🍀 **Yêu Thú Đặc Biệt (17.5% tổng):**\n` +
        ...
        (0, v2Components_1.separator)(),
        (0, v2Components_1.body)(`*Đạo hữu cần ít nhất 15 Thể Lực để thực hiện săn bắt yêu thú.*`)
    ]);
}
class SanYeuThuCommand extends Command_1.Command {
    constructor() {
