...
class TaoNhanVatCommand extends Command_1.Command {
    constructor() {
    ...
    async execute(client, interaction) {
        ...
        InteractionLock_1.InteractionLock.release(interaction.user.id);
        // Generate Linh Can early for Step 2
        const linhCanJson = CultivationService_1.cultivationService.generateLinhCan();
    ...
    async stepEpicPrologue(interaction) {
        const prologueData = [
            {
        ...
        const embeds = prologueData.map(p => new discord_js_1.EmbedBuilder()
            .setTitle(p.title)
            .setDescription(p.text)
            .setColor(p.color));
        for (let i = 0; i < embeds.length; i++) {
            await interaction.editReply((0, uiSystem_1.toV2Payload)([embeds[i]]));
            await new Promise(r => setTimeout(r, 8000));
        }
        await new Promise(r => setTimeout(r, 3000));
    }
    async stepSpiritRootCeremony(interaction, linhCan) {
    ...
    async stepChooseBackground(interaction, name) {
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields(...creationLore_1.BACKGROUNDS.map(b => ({
            name: `${b.emoji} ${b.name}`,
        ...
        const row = new discord_js_1.ActionRowBuilder().addComponents(...creationLore_1.BACKGROUNDS.map((b, i) => new discord_js_1.ButtonBuilder()
            .setCustomId(`bg_${i}`)
        ...
        const background = creationLore_1.BACKGROUNDS[idx];
        try {
        ...
        // Show intro story
        const storyEmbed = new discord_js_1.EmbedBuilder()
        ...
        await interaction.editReply((0, uiSystem_1.toV2Payload)([storyEmbed], []));
        // Brief delay for dramatic effect
        await new Promise(r => setTimeout(r, 6000));
        return background;
    }
    async stepChooseDestiny(interaction, name, background) {
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields(...creationLore_1.DESTINIES.map(d => ({
            name: `${d.emoji} ${d.name}`,
        ...
        const row = new discord_js_1.ActionRowBuilder().addComponents(...creationLore_1.DESTINIES.map((d, i) => new discord_js_1.ButtonBuilder()
            .setCustomId(`dest_${i}`)
            .setLabel(`${d.emoji} ${d.name}`)
            .setStyle(i === 0 ? discord_js_1.ButtonStyle.Danger : i === 1 ? discord_js_1.ButtonStyle.Success : discord_js_1.ButtonStyle.Secondary)));
        await interaction.editReply((0, uiSystem_1.toV2Payload)([embed], [row]));
        ...
        const idx = parseInt(collected.customId.split('_')[1], 10);
        const destiny = creationLore_1.DESTINIES[idx];
        try {
            await collected.update({ components: [] });
        }
        catch {
        ...
        // Show destiny line
        const lineEmbed = new discord_js_1.EmbedBuilder()
            .setTitle(`${destiny.emoji} ${destiny.name}`)
            .setColor(uiSystem_1.EMBED_COLORS.SUCCESS)
            .setDescription(`*"${destiny.line}"*`);
        await interaction.editReply((0, uiSystem_1.toV2Payload)([lineEmbed], []));
        await new Promise(r => setTimeout(r, 5000));
        return destiny;
    }
    async stepCreateCharacter(interaction, name, background, destiny, linhCan, linhCanJson) {
        const discordId = interaction.user.id;
        ...
        const baseStats = CultivationService_1.cultivationService.calculateStatsForLevel(1, linhCanJson);
        ...
        const combo = creationLore_1.COMBO_BONUSES.find(c => c.backgroundId === background.id && c.element === primaryElement);
        ...
        const prophecy = (0, creationLore_1.generateProphecy)(background.id, destiny.id, primaryElement);
        ...
        try {
            ...
            const formattedLinhCan = (0, constants_1.formatLinhCan)(linhCanJson);
            const linhCanFlavor = elements.map(e => `**${e}** (${linhCan[e]}%) — ${(0, creationLore_1.getLinhCanFlavorText)(e, linhCan[e])}`).join('\n');
            const openingScene = (0, creationLore_1.getOpeningScene)(name, background.id);
            const destinyLine = (0, creationLore_1.getDestinyLine)(destiny.id);
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: '👤 Đạo Hiệu', value: `**${name}** (${background.emoji} ${background.name})`, inline: true }, { name: '✨ Cảnh Giới', value: 'Luyện Khí Kỳ — Tầng 1/38', inline: true }, { name: '☯️ Định Mệnh', value: `${destiny.emoji} ${destiny.name}`, inline: true }, { name: '📜 Căn Cơ Linh Căn', value: `${formattedLinhCan}\n${linhCanFlavor}` }, { name: '📊 Chỉ Số', value: `🩸 HP: **${totalHp}** | 🌀 MP: **${totalMp}**\n⚔️ Công Kích: **${totalAtk}** | 🛡️ Phòng Thủ: **${totalDef}**\n💥 Bạo Kích: **${(totalCrit * 100).toFixed(1)}%** | 🍀 May Mắn: **${baseStats.luck + (background.bonuses.dropRate || 0)}**` }, {
...
exports.default = TaoNhanVatCommand;
