...
exports.repairAllEquipped = repairAllEquipped;
exports.handleRepairAllButton = handleRepairAllButton;
...
class SuaChuaCommand extends Command_1.Command {
    ...
    async showEquipmentList(interaction, userId, equippedItems) {
        ...
        const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            ...
            .setDisabled(equippedItems.length === 0), new discord_js_1.ButtonBuilder()
     ...
     */
    repairSingleItem(userId, item, inventory) {
        ...
        if (repairStones.length > 0) {
            ...
            if (usedStone) {
                ...
                const repairTx = database_1.default.transaction(() => {
                    InventoryRepository_1.inventoryRepository.removeItemById(usedStone.id, 1);
                    InventoryRepository_1.inventoryRepository.updateDurability(item.id, newDurability);
                });
                repairTx();
        ...
        const repairTx = database_1.default.transaction(() => {
            ...
            InventoryRepository_1.inventoryRepository.repairItem(item.id);
        });
        repairTx();
        return {
            success: true,
 ...
 */
function handleRepairAllButton(userId) {
    const inventory = InventoryRepository_1.inventoryRepository.getUserInventory(userId);
    const equippedItems = inventory.filter(i => i.is_equipped === 1);
