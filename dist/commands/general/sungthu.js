...
function getSungThuEmbed(userId, page = 1) {
    ...
    if (allPets.length === 0) {
        return (0, v2Components_1.container)(v2Components_1.V2_COLORS.info, [
            (0, v2Components_1.header)(`🐾 LINH THÚ CÁC - ${user?.name || 'Không xác định'}`, 'Sủng thú trợ chiến giúp tăng sát thương khi công kích Boss Thế Giới và vượt phó bản Bí Cảnh.'),
            (0, v2Components_1.separator)(),
            (0, v2Components_1.body)('*Đạo hữu hiện chưa thu phục được linh thú nào. Hãy sử dụng lệnh `/sanyeuthu` dã ngoại để tìm bắt linh thú!*')
        ]);
    }
    ...
    for (const pet of pets) {
        ...
        content.push((0, v2Components_1.body)(`🐾 **${pet.name}${starStr}** (Cấp ${pet.level}) [${pet.rarity.toUpperCase()}]\n` +
    ...
    if (totalPages > 1) {
        content.push((0, v2Components_1.separator)());
        content.push((0, v2Components_1.body)(`*📄 Trang ${currentPage}/${totalPages} • Tổng số: ${allPets.length} linh thú*`));
    }
    return (0, v2Components_1.container)(v2Components_1.V2_COLORS.info, content);
}
...
🌟 **Dị Biến Xảy Ra!** Phẩm chất vượt trội: **${childRarity.toUpperCase()}** (tăng từ ${pet1.rarity.toUpperCase()})!`
        ...
        if (sub === 'dokiep') {
            ...
            const curSkills = JSON.parse(pet.skills || '[]');
            ...
            const protectItem = database_1.default.prepare('SELECT quantity FROM inventory WHERE user_id = ? AND item_id = ?').get(userId, itemConstants_1.ITEMS.PILL_PET_TRIBULATION_PROTECT);
            const hasProtect = protectItem && protectItem.quantity > 0;
            // Lôi Kiếp: 5 đạo sét
            let survivedBolts = 0;
            const tribulationLog = [];
            for (let bolt = 1; bolt <= 5; bolt++) {
                ...
                if (hasProtect)
                    survivalRate += 0.20; // Hộ Thú Đan +20%
            ...
            if (hasProtect) {
                InventoryRepository_1.inventoryRepository.removeItem(userId, itemConstants_1.ITEMS.PILL_PET_TRIBULATION_PROTECT, 1);
            }
            if (survivedBolts === 5) {
                ...
                database_1.default.transaction(() => {
                    database_1.default.prepare('UPDATE pets SET rarity = ?, skills = ?, name = ? || \' 👑\' WHERE id = ?')
                ...
                database_1.default.prepare('UPDATE pets SET level = ? WHERE id = ?').run(newLevel, pet.id);
                await interaction.editReply({
