...
class DungCommand extends Command_1.Command {
    ...
    async execute(client, interaction) {
        ...
        if (userItem.type === 'chest') {
            ...
            const openTx = database_1.default.transaction(() => {
                // Trừ rương
                InventoryRepository_1.inventoryRepository.removeItem(userId, itemId, qty);
                // Thêm các phần thưởng
                for (const reward of openResult.rewards) {
                    InventoryRepository_1.inventoryRepository.addItem(userId, reward.itemId, reward.quantity, reward.customStats);
                }
            });
            openTx();
