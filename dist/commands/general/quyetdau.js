...
class QuyetAuCommand extends Command_1.Command {
    ...
    async execute(client, interaction) {
        ...
        if (action === 'lichsu') {
            ...
            const history = MinigameService_1.minigameService.getDuelHistory(userId, 1);
        ...
        if (action === 'bangxephang') {
            const leaderboard = MinigameService_1.minigameService.getDuelLeaderboard(10, challengerId);
            ...
            for (let i = 0; i < leaderboard.entries.length; i++) {
                ...
                embed.addFields({
                    ...
                    value: `🏆 **${entry.wins}** thắng | 💀 **${entry.losses}** thua | 🤝 **${entry.ties}** hòa\n` +
        ...
        if (action === 'thongke') {
            ...
            const stats = MinigameService_1.minigameService.getDuelStats(userId);
            ...
            const lastMatch = database_1.default.prepare('SELECT is_tie, winner_id FROM duel_history WHERE winner_id = ? OR loser_id = ? ORDER BY fought_at DESC LIMIT 1').get(userId, userId);
            let streakText = '';
        ...
        if (action === 'topthuong') {
            const lastSeason = MinigameService_1.minigameService.getLastSeasonTop();
            ...
            if (currentSeason.season) {
                ...
                embed.setDescription(`📅 **Mùa giải hiện tại:** #${currentSeason.season.season_number}\n` +
            ...
            if (lastSeason.season && lastSeason.top.length > 0) {
                ...
                for (const entry of lastSeason.top) {
                    ...
                    if (entry.rewardItems.length > 0) {
                        const itemNames = entry.rewardItems.map((item) => {
                            const itemRow = database_1.default.prepare('SELECT name FROM items WHERE id = ?').get(item.item_id);
                    ...
                    embed.addFields({
                        ...
                        value: `🏆 **${entry.wins}** thắng | 💰 **${entry.netWinnings >= 0 ? '+' : ''}${entry.netWinnings}** Linh Thạch ròng\n` +
                ...
                embed.setDescription('📊 **Chưa có kỳ trao thưởng nào!**\n\n' +
        ...
        const target = UserRepository_1.userRepository.get(targetUser.id);
        if (!challenger) {
            await interaction.reply({
                content: '❌ Nhân vật của đạo hữu không tồn tại! Hãy dùng `/taonhanvat` để bắt đầu.',
                ephemeral: true
     ...
     */
    buildDuelHistoryPagination(userId, page, totalPages) {
        if (totalPages <= 1)
            return null;
        const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
