...
class TongMonCommand extends Command_1.Command {
    constructor() {
        super(new discord_js_1.SlashCommandBuilder()
            ...
            .addSubcommand(sub => sub.setName('taolap').setDescription('Tạo Tông Môn mới (Phí: 50,000 Linh Thạch)')
            ...
            .addSubcommand(sub => sub.setName('thongtin').setDescription('Xem thông tin Tông Môn hiện tại (hoặc của một Tông Môn khác)')
            .addStringOption(opt => opt.setName('ten').setDescription('Tên Tông Môn (bỏ trống để xem tông môn của mình)')))
            .addSubcommand(sub => sub.setName('gia_nhap').setDescription('Xin gia nhập một Tông Môn')
            .addStringOption(opt => opt.setName('ten').setDescription('Tên Tông Môn cần xin gia nhập').setRequired(true)))
            .addSubcommand(sub => sub.setName('roi').setDescription('Rời khỏi Tông Môn hiện tại'))
            .addSubcommand(sub => sub.setName('conghien').setDescription('Quyên góp Linh Thạch vào Quỹ Tông Môn để lấy Cống Hiến')
            .addIntegerOption(opt => opt.setName('sotien').setDescription('Số Linh Thạch muốn quyên góp').setRequired(true).setMinValue(100)))
            .addSubcommand(sub => sub.setName('xaydung').setDescription('Nâng cấp Lãnh Địa (Chỉ dành cho Tông Chủ)')
            .addStringOption(opt => opt.setName('congtrinh')
            .setDescription('Công trình muốn nâng cấp')
            .setRequired(true)
            .addChoices({ name: 'Lò Rèn Bang', value: 'loren' }, { name: 'Linh Điền', value: 'linhdien' }, { name: 'Tàng Kinh Các', value: 'tangkinhcac' })))
            .addSubcommand(sub => sub.setName('thongke').setDescription('Thống kê đóng góp và xếp hạng trong Tông Môn'))
            .addSubcommandGroup(group => group.setName('tthi').setDescription('Giải đấu nội bộ Tông Môn')
            .addSubcommand(sub => sub.setName('batdau').setDescription('[Tông Chủ] Mở giải đấu nội bộ'))
            .addSubcommand(sub => sub.setName('thamgia').setDescription('Đăng ký tham gia giải đấu nội bộ'))
            .addSubcommand(sub => sub.setName('ketthuc').setDescription('[Tông Chủ] Kết thúc giải đấu và nhận thưởng'))
            .addSubcommand(sub => sub.setName('thongtin').setDescription('Xem thông tin giải đấu nội bộ'))
            .addSubcommand(sub => sub.setName('rut').setDescription('Rút khỏi giải đấu nội bộ')))
            .addSubcommandGroup(group => group.setName('lienminh').setDescription('Liên minh Tông Môn')
            .addSubcommand(sub => sub.setName('moi').setDescription('Gửi lời mời liên minh đến một Tông Môn')
            .addStringOption(opt => opt.setName('ten').setDescription('Tên Tông Môn muốn kết minh').setRequired(true)))
            .addSubcommand(sub => sub.setName('chapnhan').setDescription('Chấp nhận lời mời liên minh'))
    ...
    async execute(client, interaction) {
        ...
        if (sub === 'taolap') {
            ...
            const existing = database_1.default.prepare('SELECT id FROM sects WHERE name = ?').get(name);
            if (existing) {
            ...
            database_1.default.transaction(() => {
                ...
                const result = database_1.default.prepare('INSERT INTO sects (name, master_id, description, buildings, created_at) VALUES (?, ?, ?, ?, ?)')
                    .run(name, userId, desc, JSON.stringify(buildings), now);
                UserRepository_1.userRepository.update(userId, { sect_id: result.lastInsertRowid, sect_role: 'master', sect_contribution: 0 });
            })();
        ...
        if (sub === 'gia_nhap') {
            ...
            const sect = database_1.default.prepare('SELECT id FROM sects WHERE name = ?').get(name);
            if (!sect) {
                await interaction.editReply({ content: '❌ Không tìm thấy Tông Môn này trên giang hồ!' });
                return;
            }
            // Hiện tại cho gia nhập tự do (không cần duyệt)
            UserRepository_1.userRepository.update(userId, { sect_id: sect.id, sect_role: 'member', sect_contribution: 0 });
        ...
        if (sub === 'thongtin') {
            ...
            let sect;
            if (targetName) {
                sect = database_1.default.prepare('SELECT * FROM sects WHERE name = ?').get(targetName);
            }
            else if (user.sect_id) {
                sect = database_1.default.prepare('SELECT * FROM sects WHERE id = ?').get(user.sect_id);
            }
            if (!sect) {
                await interaction.editReply({ content: '❌ Không tìm thấy thông tin Tông Môn!' });
                return;
            }
            const master = database_1.default.prepare('SELECT name FROM users WHERE discord_id = ?').get(sect.master_id);
            const memberCount = database_1.default.prepare('SELECT COUNT(*) as c FROM users WHERE sect_id = ?').get(sect.id);
        ...
        if (sub === 'conghien') {
            ...
            database_1.default.transaction(() => {
                ...
                database_1.default.prepare('UPDATE sects SET resources = resources + ? WHERE id = ?').run(points, user.sect_id);
            })();
        ...
        if (sub === 'xaydung') {
            ...
            database_1.default.transaction(() => {
                database_1.default.prepare('UPDATE sects SET resources = resources - ?, buildings = ? WHERE id = ?')
        ...
        if (sub === 'thongke') {
            ...
            const topExp = database_1.default.prepare(`
      ...
      `).all(user.sect_id);
            // Top 3 Linh Thạch
            const topLT = database_1.default.prepare(`
      ...
      `).all(user.sect_id);
            // Top 3 Boss damage
            const topBoss = database_1.default.prepare(`
      ...
      `).all(user.sect_id);
            ...
            const barData = database_1.default.prepare(`
      ...
      `).all(user.sect_id);
          ...
          `).all(user.sect_id);
                    ...
                    database_1.default.transaction(() => {
                        top3.forEach((m, i) => {
                            if (i < 3) {
                                UserRepository_1.userRepository.update(m.discord_id, { sect_contribution: m.sect_contribution + rewards[i] });
                            }
                        });
                        database_1.default.prepare('UPDATE sects SET last_weekly_bonus_at = ? WHERE id = ?').run(nowUnix, sect.id);
                    })();
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: '🥇 Top Đóng Góp EXP', value: topExp.map((m, i) => `#${i + 1} **${m.name}** (Cấp ${m.level}): ${m.sect_contribution} điểm`).join('\n') || '*Chưa có dữ liệu*', inline: true }, { name: '💎 Top Đóng Góp Linh Thạch', value: topLT.map((m, i) => `#${i + 1} **${m.name}** (Cấp ${m.level}): ${m.coin_ha_pham} LT`).join('\n') || '*Chưa có dữ liệu*', inline: true }, { name: '🐉 Top Săn Boss', value: topBoss.map((m, i) => `#${i + 1} **${m.name}** (Cấp ${m.level}): ${m.total_damage} dmg`).join('\n') || '*Chưa có dữ liệu*', inline: true }, { name: '📈 Biểu Đồ Đóng Góp (Top 5)', value: chartLines.join('\n') || '*Chưa có dữ liệu*' })
        ...
        if (group === 'tthi') {
            ...
            if (sub === 'batdau') {
                ...
                const result = GuildWarService_1.guildWarService.startTournament(user.sect_id, userId);
                await interaction.editReply({ content: result.success ? '✅ ' + result.message : '❌ ' + result.message });
                return;
            }
            if (sub === 'thamgia') {
                const userName = user.name;
                const result = GuildWarService_1.guildWarService.joinTournament(user.sect_id, userId, userName);
                await interaction.editReply({ content: result.success ? '✅ ' + result.message : '❌ ' + result.message });
                return;
            }
            if (sub === 'ketthuc') {
                ...
                const result = GuildWarService_1.guildWarService.endTournament(user.sect_id, userId);
                await interaction.editReply({ content: result.success ? result.message : '❌ ' + result.message });
                return;
            }
            if (sub === 'thongtin') {
                const info = GuildWarService_1.guildWarService.getTournamentInfo(user.sect_id);
                if (!info) {
            ...
            if (sub === 'rut') {
                const result = GuildWarService_1.guildWarService.leaveTournament(user.sect_id, userId);
                await interaction.editReply({ content: result.success ? '✅ ' + result.message : '❌ ' + result.message });
                return;
            }
        }
        if (group === 'lienminh') {
            ...
            if (sub === 'moi') {
                ...
                const targetSect = database_1.default.prepare('SELECT id, name FROM sects WHERE name = ?').get(targetName);
                if (!targetSect) {
                    await interaction.editReply({ content: '❌ Không tìm thấy Tông Môn **' + targetName + '** trên giang hồ!' });
                    return;
                }
                const result = SectService_1.sectService.formAlliance(user.sect_id, targetSect.id, userId);
                await interaction.editReply({ content: result.success ? '✅ ' + result.message : '❌ ' + result.message });
                return;
            }
            if (sub === 'chapnhan') {
                const result = SectService_1.sectService.acceptAlliance(user.sect_id, userId);
                await interaction.editReply({ content: result.success ? '✅ ' + result.message : '❌ ' + result.message });
                return;
            }
            if (sub === 'huy') {
                const result = SectService_1.sectService.breakAlliance(user.sect_id, userId);
                await interaction.editReply({ content: result.success ? result.message : '❌ ' + result.message });
                return;
            }
            if (sub === 'thongtin') {
                const data = SectService_1.sectService.getAlliance(user.sect_id);
                if (!data) {
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: '👑 Tông Chủ', value: data.partnerSect.master_name, inline: true }, { name: '📊 Cấp Độ', value: `${data.partnerSect.level}`, inline: true }, { name: '👥 Thành Viên', value: `${data.partnerSect.member_count}`, inline: true }, { name: '📅 Kết Minh Từ', value: `<t:${data.alliance.formed_at}:R>` });
                await interaction.editReply((0, uiSystem_1.toV2Payload)([embed]));
                return;
            }
            if (sub === 'tuyen-chien') {
                ...
                const result = SectService_1.sectService.declareWar(userId, user.sect_id, targetSect.id);
                await interaction.editReply({ content: result.success ? result.message : '❌ ' + result.message });
                return;
            }
        }
