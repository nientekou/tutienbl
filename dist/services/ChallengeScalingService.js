...
exports.challengeScalingService = void 0;
...
class ChallengeScalingService {
     ...
     */
    applyModifiers(playerStats, enemyStats, modifiers) {
     ...
     */
    getChallengeDescription(modifiers) {
        if (modifiers.length === 0)
            return 'Không có modifier thử thách nào đang hoạt động.';
     ...
     */
    getRewardMultiplier(modifiers) {
        let mult = 1.0;
        for (const mod of modifiers) {
            if (mod.difficulty === 'hard')
                mult += 0.5;
            else if (mod.difficulty === 'medium')
                mult += 0.25;
            else
                mult += 0.1;
        }
...
exports.challengeScalingService = new ChallengeScalingService();
