...
class EquipmentService {
     ...
     */
    appraisePhoi(userId, inventoryId, qty = 1) {
        ...
        const appraiseTx = database_1.default.transaction(() => {
            // Trừ Linh thạch
            database_1.default.prepare('UPDATE users SET coin_ha_pham = coin_ha_pham - ? WHERE discord_id = ?').run(cost, userId);
            // Xóa phôi
     ...
     */
    salvageEquipment(userId, inventoryId, qty = 1) {
        const item = InventoryRepository_1.inventoryRepository.get(inventoryId);
        if (!item || item.user_id !== userId) {
     ...
     */
    salvageEquipmentBulk(userId, targetRarity) {
        const RARITY_ORDER = {
            common: 1,
            uncommon: 2,
            rare: 3,
        ...
        const allItems = InventoryRepository_1.inventoryRepository.getUserInventory(userId);
        const salvageable = allItems.filter(item => {
            if (item.equipable !== 1)
                return false;
            if (item.is_equipped === 1)
                return false;
            if (item.is_life_bound === 1)
                return false;
        ...
        const salvageTx = database_1.default.transaction(() => {
            for (const invId of itemsToRemove) {
                ...
                const it = InventoryRepository_1.inventoryRepository.get(invId);
                if (it) {
     ...
     * Nâng Sao Trang Bị
     */
    upgradeStars(userId, inventoryId) {
        const item = InventoryRepository_1.inventoryRepository.get(inventoryId);
        if (!item || item.user_id !== userId) {
    ...
    appraisePhoiBulk(userId) {
        ...
        const appraiseTx = database_1.default.transaction(() => {
            ...
            // Xóa tất cả phôi
            for (const p of phois) {
                InventoryRepository_1.inventoryRepository.removeItemById(p.id, p.quantity);
            }
            // Thêm thành phẩm
            InventoryRepository_1.inventoryRepository.addMultipleItems(itemsToAdd);
        });
