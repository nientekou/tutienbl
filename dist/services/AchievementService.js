...
class AchievementService {
     ...
     */
    checkHiddenAchievements(userId) {
        const user = UserRepository_1.userRepository.get(userId);
        if (!user)
            return [];
        const unlocked = [];
        ...
        try {
            const towerProgress = database_1.default.prepare('SELECT max_floor FROM roguelike_progress WHERE user_id = ?').get(userId);
        ...
        try {
            const loginRecord = database_1.default.prepare('SELECT streak FROM user_daily_logins WHERE user_id = ?').get(userId);
        ...
        try {
            const beastCount = database_1.default.prepare('SELECT COUNT(*) as c FROM rare_beasts WHERE user_id = ?').get(userId);
            if (beastCount.c >= 15) {
        ...
        try {
            const gradCount = database_1.default.prepare("SELECT COUNT(*) as c FROM mentorships WHERE mentor_id = ? AND status = 'graduated'").get(userId);
            if (gradCount.c >= 10) {
    ...
    getCategoryStats(userId) {
        ...
        return categories.map(cat => {
            ...
            const completedCat = catAchievements.filter(a => {
                const ua = database_1.default.prepare('SELECT is_completed FROM user_achievements WHERE user_id = ? AND achievement_id = ?')
                    .get(userId, a.id);
     ...
     */
    getAchievementChains(userId) {
        return this.ACHIEVEMENT_CHAINS.map(chain => ({
            chainId: chain.id,
            chainName: chain.name,
            parts: chain.parts.map(part => {
                const ua = database_1.default.prepare('SELECT progress, is_completed FROM user_achievements WHERE user_id = ? AND achievement_id = ?')
                    .get(userId, part.id);
                return {
                    id: part.id,
                    name: part.name,
     ...
     */
    buyFromPointsShop(userId, itemId) {
     ...
     */
    getAchievementDescription(userId) {
    ...
    checkWeeklyAchievements(userId) {
        ...
        return weekly.map(a => {
            const ua = database_1.default.prepare('SELECT progress FROM user_achievements WHERE user_id = ? AND achievement_id = ?')
                .get(userId, a.id);
