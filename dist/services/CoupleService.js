...
class CoupleService {
    ...
    dualCultivate(coupleId) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleById(coupleId);
        if (!couple)
            return { success: false, message: 'Đạo hữu chưa kết Đạo Lữ!' };
        ...
        const u1 = UserRepository_1.userRepository.get(couple.user1_id);
        const u2 = UserRepository_1.userRepository.get(couple.user2_id);
        if (!u1 || !u2)
            return { success: false, message: 'Lỗi không tìm thấy Đạo Lữ!' };
        ...
        const tx = database_1.default.transaction(() => {
            UserRepository_1.userRepository.update(u1.discord_id, { tu_vi: u1.tu_vi + expGain, intimacy: couple.intimacy + 10, last_songtu_at: now });
            UserRepository_1.userRepository.update(u2.discord_id, { tu_vi: u2.tu_vi + expGain, intimacy: couple.intimacy + 10, last_songtu_at: now });
            CoupleRepository_1.coupleRepository.updateLastDualCultivation(coupleId, now);
            CoupleRepository_1.coupleRepository.updateIntimacy(coupleId, 10); // Tăng 10 điểm hảo cảm sau mỗi lần song tu
        });
        tx();
     ...
     */
    checkAnniversaryMilestones(couple) {
        const now = Math.floor(Date.now() / 1000);
        ...
        for (const ms of milestones) {
            ...
            database_1.default.transaction(() => {
                ...
                // User 1
                UserRepository_1.userRepository.update(u1.discord_id, {
                ...
                yCanh1[msKey] = true;
                UserRepository_1.userRepository.update(u1.discord_id, { y_canh: JSON.stringify(yCanh1) });
                let yCanh2 = {};
                try {
    ...
    checkAnniversaryOnInfo(coupleId) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleById(coupleId);
        if (!couple)
            return '';
    ...
    giveGift(coupleId, giftValue) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleById(coupleId);
        if (!couple)
            return { success: false, message: 'Đạo hữu chưa kết Đạo Lữ!' };
        CoupleRepository_1.coupleRepository.updateIntimacy(coupleId, giftValue);
        UserRepository_1.userRepository.update(couple.user1_id, { intimacy: couple.intimacy + giftValue });
        UserRepository_1.userRepository.update(couple.user2_id, { intimacy: couple.intimacy + giftValue });
     ...
     */
    getCoupleDungeonInfo(userId) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleByUserId(userId);
        if (!couple)
            return null;
        const today = new Date().toISOString().slice(0, 10);
        let dungeonRow = database_1.default.prepare('SELECT * FROM couple_dungeons WHERE couple_id = ? AND user_id = ?').get(couple.id, userId);
        if (!dungeonRow) {
            database_1.default.prepare('INSERT INTO couple_dungeons (couple_id, user_id, current_floor, daily_runs, last_run_date, best_floor) VALUES (?, ?, 1, 0, ?, 0)').run(couple.id, userId, today);
            dungeonRow = database_1.default.prepare('SELECT * FROM couple_dungeons WHERE couple_id = ? AND user_id = ?').get(couple.id, userId);
        }
        // Reset daily runs if new day
        if (dungeonRow.last_run_date !== today) {
            database_1.default.prepare('UPDATE couple_dungeons SET daily_runs = 0, current_floor = 1, last_run_date = ? WHERE couple_id = ? AND user_id = ?').run(today, couple.id, userId);
     ...
     */
    enterCoupleDungeon(userId) {
        const info = this.getCoupleDungeonInfo(userId);
        if (!info)
            return { success: false, message: 'Đạo hữu chưa có Đạo Lữ!' };
        if (info.dailyRuns >= 2) {
            return { success: false, message: 'Đã hết lượt Đôi Bí Cảnh hôm nay (2/2).' };
        }
        database_1.default.prepare('UPDATE couple_dungeons SET daily_runs = daily_runs + 1 WHERE couple_id = ? AND user_id = ?').run(info.coupleId, userId);
        ...
        if (floor <= 3) {
            ...
            const couple = CoupleRepository_1.coupleRepository.getCoupleById(info.coupleId);
            if (couple) {
                UserRepository_1.userRepository.update(couple.user1_id, { tu_vi: Math.min(database_1.default.prepare('SELECT tu_vi FROM users WHERE discord_id = ?').get(couple.user1_id).tu_vi + expReward, database_1.default.prepare('SELECT exp_needed FROM users WHERE discord_id = ?').get(couple.user1_id).exp_needed) });
                UserRepository_1.userRepository.update(couple.user2_id, { tu_vi: Math.min(database_1.default.prepare('SELECT tu_vi FROM users WHERE discord_id = ?').get(couple.user2_id).tu_vi + expReward, database_1.default.prepare('SELECT exp_needed FROM users WHERE discord_id = ?').get(couple.user2_id).exp_needed) });
                CoupleRepository_1.coupleRepository.updateIntimacy(info.coupleId, intimacyReward);
            }
            const newBest = Math.max(info.bestFloor, floor);
            database_1.default.prepare('UPDATE couple_dungeons SET current_floor = current_floor + 1, best_floor = ? WHERE couple_id = ? AND user_id = ?').run(newBest, info.coupleId, userId);
     ...
     */
    getCoupleDungeonDescription(userId) {
        const info = this.getCoupleDungeonInfo(userId);
        if (!info)
            return '❌ Đạo hữu chưa có Đạo Lữ để vào Đôi Bí Cảnh!';
        return `💕 **Đôi Bí Cảnh**\n━━━━━━━━━━━━━━━━━━━━━━━\n` +
    ...
    applyIntimacyDecay() {
        try {
            const couples = database_1.default.prepare('SELECT * FROM couples').all();
            const now = Math.floor(Date.now() / 1000);
            const oneDayAgo = now - 86400;
            for (const couple of couples) {
                // Check if any interaction in last 24h
                const lastInteraction = database_1.default.prepare(`
        ...
        `).get(couple.id);
                if (lastInteraction && lastInteraction.last < oneDayAgo) {
                    ...
                    if (decay > 0) {
                        database_1.default.prepare('UPDATE couples SET intimacy = MAX(0, intimacy - ?) WHERE id = ?')
                            .run(decay, couple.id);
                    }
                }
            }
     ...
     */
    getCoupleAchievements(userId) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleByUserId(userId);
        if (!couple)
            return [];
        const achievements = [];
        // Milestones
        if (couple.intimacy >= 100)
            achievements.push('💕 **Thân mật 100** — Đôi tình nhân');
        if (couple.intimacy >= 500)
            achievements.push('💖 **Thân mật 500** — Đôi vợ chồng son');
        if (couple.intimacy >= 1000)
            achievements.push('❤️‍🔥 **Thân mật 1000** — Đôi ngọctypeorm');
        if (couple.intimacy >= 5000)
            achievements.push('💕 **Thân mật 5000** — Đôi thiên nhiên');
        // Anniversary
        const daysTogether = Math.floor((Date.now() / 1000 - couple.marriage_date) / 86400);
        if (daysTogether >= 30)
            achievements.push(`📅 **30 ngày** — Kỷ niệm 1 tháng`);
        if (daysTogether >= 100)
            achievements.push(`📅 **100 ngày** — Kỷ niệm 100 ngày`);
        if (daysTogether >= 365)
            achievements.push(`📅 **365 ngày** — Kỷ niệm 1 năm`);
        // Dungeon
        try {
            const dungeonInfo = database_1.default.prepare('SELECT MAX(best_floor) as best FROM couple_dungeons WHERE couple_id = ?')
                .get(couple.id);
            if (dungeonInfo && dungeonInfo.best >= 3)
                achievements.push('🏯 **Đôi Bí Cảnh** — Vượt qua 3 tầng');
        }
        catch { }
        return achievements;
    }
     ...
     */
    checkAnniversaryMilestonesV2(userId) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleByUserId(userId);
        if (!couple)
            return '';
        const daysTogether = Math.floor((Date.now() / 1000 - couple.marriage_date) / 86400);
     ...
     */
    useJointSkill(userId, skillType) {
        const couple = CoupleRepository_1.coupleRepository.getCoupleByUserId(userId);
        if (!couple)
            return { success: false, message: '❌ Chưa có Đạo Lữ!' };
        const user = UserRepository_1.userRepository.get(userId);
        if (!user)
        ...
        if (skill.cost > 0) {
            UserRepository_1.userRepository.update(userId, { stamina: user.stamina - skill.cost });
        }
        return {
            success: true,
