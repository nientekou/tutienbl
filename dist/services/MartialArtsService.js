...
class MartialArtsService {
    ...
    learnArt(userId, artId) {
        ...
        if (!art)
            return { success: false, message: 'Võ công không tồn tại!' };
        ...
        const existing = database_1.default.prepare('SELECT * FROM user_martial_arts WHERE user_id = ? AND art_id = ?')
            .get(userId, artId);
        if (existing)
            return { success: false, message: 'Đã học võ công này rồi!' };
    ...
    fuseArts(userId, art1Id, art2Id) {
        ...
        if (!fusion)
            return { success: false, message: 'Các võ công này không thể dung hợp!' };
        ...
        if (!art1 || !art2)
            return { success: false, message: 'Cần cả hai võ công để dung hợp!' };
        // Check success rate
