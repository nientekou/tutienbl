...
class TitleService {
    ...
    unlockTitle(userId, titleId) {
        ...
        if (!title)
            return { success: false, message: 'Danh hiệu không tồn tại' };
        const existing = database_1.default.prepare('SELECT 1 FROM user_titles WHERE user_id = ? AND title_id = ?').get(userId, titleId);
        if (existing)
            return { success: false, message: 'Danh hiệu này đã được mở khoá' };
        database_1.default.prepare('INSERT INTO user_titles (user_id, title_id, unlocked_at) VALUES (?, ?, ?)')
    ...
    equipTitle(userId, titleId) {
        ...
        if (!user)
            return { success: false, message: 'Người dùng không tồn tại' };
        const owned = database_1.default.prepare('SELECT 1 FROM user_titles WHERE user_id = ? AND title_id = ?').get(userId, titleId);
        if (!owned)
            return { success: false, message: 'Đạo hữu chưa sở hữu danh hiệu này' };
        const title = TITLES.find(t => t.id === titleId);
        database_1.default.prepare('INSERT OR REPLACE INTO user_title_config (user_id, equipped_title) VALUES (?, ?)')
            .run(userId, titleId);
        return { success: true, message: `🏅 Đã trang bị danh hiệu: **${title?.name || titleId}**!` };
    }
    getTitleBonuses(userId) {
        this.initTable();
        const config = database_1.default.prepare('SELECT equipped_title FROM user_title_config WHERE user_id = ?').get(userId);
    ...
    getOwnedTitles(userId) {
        this.initTable();
        const rows = database_1.default.prepare('SELECT title_id FROM user_titles WHERE user_id = ?').all(userId);
        return rows.map(r => r.title_id);
    }
    getTitleDescription(userId) {
        this.initTable();
        const config = database_1.default.prepare('SELECT equipped_title FROM user_title_config WHERE user_id = ?').get(userId);
        ...
        const categories = ['combat', 'social', 'crafting', 'exploration', 'special'];
