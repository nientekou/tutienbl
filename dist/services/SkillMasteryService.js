...
exports.skillMasteryService = void 0;
...
class SkillMasteryService {
     ...
     */
    getMastery(userId, skillId) {
        this.initTable();
        let row = database_1.default.prepare('SELECT * FROM skill_mastery WHERE user_id = ? AND skill_id = ?')
            .get(userId, skillId);
        if (!row) {
            database_1.default.prepare('INSERT INTO skill_mastery (user_id, skill_id, mastery_level, mastery_exp) VALUES (?, ?, 1, 0)')
                .run(userId, skillId);
            row = database_1.default.prepare('SELECT * FROM skill_mastery WHERE user_id = ? AND skill_id = ?')
                .get(userId, skillId);
        }
        return row;
    }
     ...
     */
    addMasteryExp(userId, skillId, exp = 10) {
        this.initTable();
        ...
        if (newExp >= needed && mastery.mastery_level < MAX_MASTERY_LEVEL) {
            ...
            database_1.default.prepare('UPDATE skill_mastery SET mastery_level = ?, mastery_exp = ? WHERE user_id = ? AND skill_id = ?')
                .run(newLevel, newExp - needed, userId, skillId);
            return { levelUp: true, newLevel };
        }
        database_1.default.prepare('UPDATE skill_mastery SET mastery_exp = ? WHERE user_id = ? AND skill_id = ?')
     ...
     */
    choosePath(userId, skillId, path) {
        this.initTable();
        const mastery = this.getMastery(userId, skillId);
        ...
        database_1.default.prepare('UPDATE skill_mastery SET chosen_path = ? WHERE user_id = ? AND skill_id = ?')
     ...
     */
    getMasteryBonus(userId, skillId) {
        const mastery = this.getMastery(userId, skillId);
     ...
     */
    getMasteryDescription(userId, skillId) {
        const mastery = this.getMastery(userId, skillId);
        const needed = MASTERY_EXP_PER_LEVEL * mastery.mastery_level;
...
exports.skillMasteryService = new SkillMasteryService();
