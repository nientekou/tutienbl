...
class AutoBalanceService {
    ...
    `).all();
        ...
        const powers = users.map(u => {
            return Math.round((u.base_hp || 100) * 0.2 +
                (u.base_mp || 50) * 0.1 +
                (u.base_atk || 15) * 3 +
                (u.base_def || 10) * 5 +
    ...
    getPvPMultipliers(userId) {
        ...
        const user = database_1.default.prepare('SELECT base_atk, base_def, base_hp, base_mp, base_crit, base_crit_res, base_luck, base_speed, base_dodge FROM users WHERE discord_id = ?').get(userId);
        if (!user)
            return { atkMult: 1.0, defMult: 1.0 };
        const userPower = Math.round((user.base_hp || 100) * 0.2 + (user.base_mp || 50) * 0.1 +
