...
class CombatEngine {
    ...
    static run(player, enemy, pet = null, maxRounds = 30, isDreamscape = false, isSurvival = false, guildId) {
        ...
        if (player.userId) {
            try {
                ...
                if (daoBonuses.atk_bonus)
                    player.atk = Math.floor(player.atk * (1 + daoBonuses.atk_bonus / 100));
                if (daoBonuses.crit_bonus)
                    player.crit += daoBonuses.crit_bonus;
                if (daoBonuses.dodge_bonus)
                    player.dodge = (player.dodge ?? 0.05) + daoBonuses.dodge_bonus / 100;
                if (daoBonuses.hp_bonus) {
                    player.maxHp += daoBonuses.hp_bonus;
                    player.hp += daoBonuses.hp_bonus;
                }
                if (daoBonuses.mp_regen)
                    player.mp = (player.mp ?? 0) + daoBonuses.mp_regen;
                if (daoBonuses.def_bonus)
                    player.def = Math.floor(player.def * (1 + daoBonuses.def_bonus / 100));
            }
            catch { }
        }
        // === RARE BEAST BONUSES ===
        if (player.userId) {
            try {
                ...
                if (beastBonuses.atk)
                    player.atk += beastBonuses.atk;
                if (beastBonuses.def)
                    player.def += beastBonuses.def;
                if (beastBonuses.hp) {
                    player.maxHp += beastBonuses.hp;
                    player.hp += beastBonuses.hp;
                }
                ...
                if (beastBonuses.passive === 'crit_hunt')
                    player.crit += 0.08;
                if (beastBonuses.passive === 'metal_resist')
                    player.def = Math.round(player.def * 1.10);
            }
            catch { }
        }
        // DoT Ticks
        ...
        if (player.userId) {
            try {
                ...
                if (form) {
                    ...
                    if (sw?.element === 'Hỏa' || sw?.element === 'Kim')
                        player.atk = Math.round(player.atk * 1.15);
                    else if (sw?.element === 'Thủy' || sw?.element === 'Mộc') {
                        player.maxHp = Math.round(player.maxHp * 1.15);
                        player.hp = Math.round(player.hp * 1.15);
                    }
                    else if (sw?.element === 'Thổ')
                        player.def = Math.round(player.def * 1.15);
                    else if (sw?.element === 'Lôi')
                        player.crit += 0.05;
                    else if (sw?.element === 'Phong') {
                        player.speed = Math.round((player.speed ?? 100) * 1.15);
                        player.dodge = (player.dodge ?? 0.05) + 0.08;
                    }
                }
            }
            catch { }
        ...
        let skillMasteryAwakenedEffects = {};
        if (player.userId) {
            try {
                ...
                for (const sk of skills) {
                    ...
                    if (bonus.awakenedEffect)
                        skillMasteryAwakenedEffects[sk.id] = bonus.awakenedEffect;
                }
            }
            catch { }
        }
        // === V14 D-03: WEATHER ELEMENTAL BONUS ===
        let weatherElementBonus = null;
            ...
            const executePlayerTurn = () => {
                ...
                for (const [element, percentage] of Object.entries(elements)) {
                    ...
                    if (percentage >= 90)
                        ...
                        triggerChance = 0.0; // Phế Linh Căn: 0%
                ...
                if (enemyHp > 0) {
                    ...
                    if (isEnemyDodge) {
                        ...
                        if (activeSkill) {
                            ...
                            if (awakenedEffect) {
                                if (awakenedEffect === 'fire_burst')
                                    baseDamage = Math.round(baseDamage * 1.30);
                                else if (awakenedEffect === 'fire_aoe')
                                    baseDamage = Math.round(baseDamage * 1.50); // AOE simulated as +50% single target
                                else if (awakenedEffect === 'water_penetrate') {
                                    enemyDef = Math.round(enemyDef * 0.70);
                                    baseDamage = Math.max(1, Math.round(player.atk * (1 - Math.min(0.70, enemyDef / (player.atk * 0.7 + enemyDef)))));
                                }
                                else if (awakenedEffect === 'wood_lifesteal_plus')
                                    playerLifesteal += 0.15;
                                else if (awakenedEffect === 'earth_stun_burst')
                                    enemyParalyzed = true;
                                else if (awakenedEffect === 'lightning_burst')
                                    baseDamage = Math.round(baseDamage * 1.50);
                                else if (awakenedEffect === 'lightning_chain')
                                    baseDamage = Math.round(baseDamage * 2.40); // 3 hits x 80%
                                else
                                    baseDamage = Math.round(baseDamage * 1.20); // generic +20%
                            }
                            ...
                            if (enemy.element) {
                                ...
                                const ee = enemy.element;
                                let elemMult = 1.0;
                                const isAdvantage = (pe === 'Kim' && ee === 'Mộc') ||
                                    (pe === 'Mộc' && ee === 'Thổ') ||
                                    (pe === 'Thổ' && ee === 'Thủy') ||
                                    (pe === 'Thủy' && ee === 'Hỏa') ||
                                    (pe === 'Hỏa' && ee === 'Kim');
                                const isDisadvantage = (ee === 'Kim' && pe === 'Mộc') ||
                                    (ee === 'Mộc' && pe === 'Thổ') ||
                                    (ee === 'Thổ' && pe === 'Thủy') ||
                                    (ee === 'Thủy' && pe === 'Hỏa') ||
                                    (ee === 'Hỏa' && pe === 'Kim');
                ...
                if (enemyParalyzed) {
                    ...
                    if (isPlayerDodge) {
                        ...
                        if (player.bloodline && player.bloodline.passives.dmg_reduce) {
                            let mult = 1.0;
                            if (rageActive)
                                mult = player.bloodline.rage_effect.multiplier || 2;
        ...
        let winner = playerHp > 0 ? 'player' : 'enemy';
        if (isSurvival) {
            if (playerHp > 0) {
                winner = 'player';
    ...
    static getAutoBattleSettings(userId) {
        try {
            const db = require('../database/database').default;
            const row = db.prepare('SELECT auto_battle_settings FROM users WHERE discord_id = ?').get(userId);
    ...
    static setAutoBattleSettings(userId, settings) {
        try {
            const db = require('../database/database').default;
