...
class SoulWeaponService {
    ...
    getEvolutionStage(level) {
        for (let i = EVOLUTION_STAGES.length - 1; i >= 0; i--) {
            if (level >= EVOLUTION_STAGES[i].levelRange[0])
    ...
    canEvolve(userId) {
        ...
        if (nextStageIdx === -1)
            return { canEvolve: false }; // Already max stage
    ...
    evolveWeapon(userId) {
        const sw = SoulWeaponRepository_1.soulWeaponRepository.getByUserId(userId);
        if (!sw)
            return { success: false, message: '❌ Không có Pháp Bảo!' };
        const currentStage = this.getEvolutionStage(sw.level);
        const nextStageIdx = EVOLUTION_STAGES.findIndex(s => s.stage === currentStage.stage + 1);
        if (nextStageIdx === -1)
            return { success: false, message: '❌ Đã đạt giai đoạn tối đa!' };
        const nextStage = EVOLUTION_STAGES[nextStageIdx];
        if (sw.level < nextStage.levelRange[0]) {
        ...
        SoulWeaponRepository_1.soulWeaponRepository.updateEvolution(sw.id, nextStage.stage);
        return {
            success: true,
    ...
    awakenWeapon(userId) {
        ...
        SoulWeaponRepository_1.soulWeaponRepository.updateAwakening(sw.id, currentAwakenings + 1);
        return {
            success: true,
     ...
     */
    getWeaponDescription(userId) {
        const sw = SoulWeaponRepository_1.soulWeaponRepository.getByUserId(userId);
        if (!sw)
            return '❌ Đạo hữu chưa có Pháp Bảo Bản Mệnh.';
        const stage = this.getEvolutionStage(sw.level);
