...
class CompanionService {
    ...
    getUnlockedCompanions(userId) {
        this.initTable();
        const rows = database_1.default.prepare('SELECT companion_type FROM companion WHERE user_id = ?').all(userId);
        const unlocked = new Set(rows.map(r => r.companion_type));
        return COMPANIONS.filter(c => {
            if (c.unlockReq === 'start')
                return true;
            return unlocked.has(c.type);
        });
    }
    getCompanion(userId, type) {
        this.initTable();
        return database_1.default.prepare('SELECT level, xp, evolved FROM companion WHERE user_id = ? AND companion_type = ?')
            .get(userId, type) || null;
    }
    getEquipped(userId) {
        this.initTable();
        const row = database_1.default.prepare('SELECT companion_type FROM companion WHERE user_id = ? AND equipped = 1')
            .get(userId);
        if (!row)
            return null;
    ...
    equip(userId, type) {
        ...
        if (!existing)
            return { success: false, message: 'Chưa sở hữu companion này.' };
        database_1.default.prepare('UPDATE companion SET equipped = 0 WHERE user_id = ?').run(userId);
        database_1.default.prepare('UPDATE companion SET equipped = 1 WHERE user_id = ? AND companion_type = ?').run(userId, type);
        return { success: true, message: `Đã trang bị ${COMPANIONS.find(c => c.type === type)?.name || type}.` };
    }
    addXp(userId, type, amount) {
        this.initTable();
        const comp = this.getCompanion(userId, type);
        if (!comp)
            return { levelUp: false, newLevel: 1, evolved: false };
        ...
        if (level >= MAX_LEVEL && !comp.evolved) {
            database_1.default.prepare('UPDATE companion SET evolved = 1 WHERE user_id = ? AND companion_type = ?').run(userId, type);
            evolved = true;
        }
        database_1.default.prepare('UPDATE companion SET level = ?, xp = ? WHERE user_id = ? AND companion_type = ?')
            .run(level, xp, userId, type);
        return { levelUp, newLevel: level, evolved };
    }
    unlock(userId, type) {
        this.initTable();
        if (this.getCompanion(userId, type))
            return { success: false, message: 'Đã sở hữu.' };
        database_1.default.prepare('INSERT INTO companion (user_id, companion_type, level, xp, evolved, equipped) VALUES (?, ?, 1, 0, 0, 0)')
            .run(userId, type);
        const def = COMPANIONS.find(c => c.type === type);
        return { success: true, message: `Đã mở khóa **${def?.name || type}**!` };
    }
    getCombatPassive(userId) {
...
exports.companionService = new CompanionService();
