...
class InventoryService {
     ...
     */
    computeElementResonance(userId) {
        const user = UserRepository_1.userRepository.get(userId);
        if (!user)
            return { element: '', resonance: false, buffs: [] };
        ...
        const equippedSkills = database_1.default.prepare("SELECT skill_id FROM user_skills WHERE user_id = ? AND is_equipped = 1").all(userId);
        const SKILL_DETAILS = {
        ...
        const equippedHeartLaws = database_1.default.prepare(`
    ...
    getActiveStats(userId) {
        ...
        for (const item of equippedItems) {
            try {
                ...
                if (bonus.crit_res)
                    stats.critRes += bonus.crit_res;
                if (bonus.luck)
                    stats.luck += bonus.luck;
                if (bonus.speed)
                    stats.speed += bonus.speed;
                if (bonus.dodge)
                    stats.dodge += bonus.dodge;
                if (bonus.hp_percent)
                    multipliers.hp += bonus.hp_percent;
                if (bonus.mp_percent)
                    multipliers.mp += bonus.mp_percent;
                if (bonus.atk_percent)
                    multipliers.atk += bonus.atk_percent;
                if (bonus.def_percent)
                    multipliers.def += bonus.def_percent;
                if (bonus.speed_percent)
                    multipliers.speed += bonus.speed_percent;
                // Cộng thêm từ chỉ số rèn ngẫu nhiên (nếu có)
                if (item.custom_stats) {
                    const customBonus = JSON.parse(item.custom_stats);
                    if (customBonus.hp)
                        stats.hp += Math.round(customBonus.hp * durabilityMult * boundMult);
                    if (customBonus.mp)
                        stats.mp += Math.round(customBonus.mp * durabilityMult * boundMult);
                    if (customBonus.atk)
                        stats.atk += Math.round(customBonus.atk * durabilityMult * boundMult);
                    if (customBonus.def)
                        stats.def += Math.round(customBonus.def * durabilityMult * boundMult);
                    if (customBonus.crit)
                        stats.crit += customBonus.crit * durabilityMult * boundMult;
                    if (customBonus.luck)
                        stats.luck += Math.round(customBonus.luck * durabilityMult * boundMult);
                    if (customBonus.speed)
                        stats.speed += Math.round(customBonus.speed * durabilityMult * boundMult);
                    if (customBonus.dodge)
                        stats.dodge += customBonus.dodge * durabilityMult * boundMult;
                    if (customBonus.hp_percent)
                        multipliers.hp += customBonus.hp_percent * durabilityMult;
                    if (customBonus.mp_percent)
                        multipliers.mp += customBonus.mp_percent * durabilityMult;
                    if (customBonus.atk_percent)
                        multipliers.atk += customBonus.atk_percent * durabilityMult;
                    if (customBonus.def_percent)
                        multipliers.def += customBonus.def_percent * durabilityMult;
                    if (customBonus.speed_percent)
                        multipliers.speed += customBonus.speed_percent * durabilityMult;
        ...
        try {
            ...
            for (const [key, val] of Object.entries(imprintStats)) {
                ...
                if (key === 'hp')
                    ...
                    stats.dodge += v;
                else if (key === 'hp_percent')
                    multipliers.hp += v;
                else if (key === 'mp_percent')
                    multipliers.mp += v;
                else if (key === 'atk_percent')
                    multipliers.atk += v;
                else if (key === 'def_percent')
                    multipliers.def += v;
        ...
        if (resonance.resonance) {
            if (resonance.atkPercent)
                multipliers.atk += resonance.atkPercent;
            if (resonance.hpPercent)
                multipliers.hp += resonance.hpPercent;
            if (resonance.defPercent)
                multipliers.def += resonance.defPercent;
            if (resonance.critBonus)
                stats.crit += resonance.critBonus;
            if (resonance.dodgeBonus)
                stats.dodge += resonance.dodgeBonus;
        ...
        try {
            ...
            if (treeBonuses.atk_bonus)
                multipliers.atk += treeBonuses.atk_bonus;
            if (treeBonuses.def_bonus)
                multipliers.def += treeBonuses.def_bonus;
            if (treeBonuses.hp_bonus)
                multipliers.hp += treeBonuses.hp_bonus;
            if (treeBonuses.speed_bonus)
                multipliers.speed += treeBonuses.speed_bonus;
            if (treeBonuses.crit_bonus)
                stats.crit += treeBonuses.crit_bonus;
            if (treeBonuses.dodge_bonus)
                stats.dodge += treeBonuses.dodge_bonus;
            if (treeBonuses.block_bonus)
                stats.block_chance = Math.min(1, (stats.block_chance || 0) + treeBonuses.block_bonus);
            if (treeBonuses.regen_bonus)
                stats.mp_regen = (stats.mp_regen || 0) + treeBonuses.regen_bonus;
        }
        catch (e) { }
    ...
    equipItem(userId, inventoryId) {
        ...
        if ((0, itemConstants_1.isWeaponId)(item.item_id))
            ...
            slot = 'ring';
        else if (item.item_id.startsWith('necklace_'))
            slot = 'necklace';
        else if (item.item_id.startsWith('amulet_'))
            slot = 'amulet';
        else if (item.item_id.startsWith('pendant_') || item.item_id.startsWith('boi_pham_'))
            slot = 'pendant';
        else if (item.item_id.startsWith('mount_'))
            slot = 'mount';
        else
            slot = 'treasure'; // Pháp bảo
        // Tìm và tháo trang bị cũ trong slot đó (nếu có)
        const currentlyEquipped = InventoryRepository_1.inventoryRepository.getEquippedInSlot(userId, slot);
        if (currentlyEquipped) {
            InventoryRepository_1.inventoryRepository.updateEquipmentStatus(currentlyEquipped.id, 0, null);
        }
        // Đeo trang bị mới
        InventoryRepository_1.inventoryRepository.updateEquipmentStatus(inventoryId, 1, slot);
        return {
            success: true,
     ...
     */
    unequipItem(userId, inventoryId) {
        const item = InventoryRepository_1.inventoryRepository.get(inventoryId);
        if (!item || item.user_id !== userId) {
        ...
        InventoryRepository_1.inventoryRepository.updateEquipmentStatus(inventoryId, 0, null);
        ...
        try {
            ...
            if (item.item_id === itemConstants_1.ITEMS.PILL_ALCHEMY_STAMINA || item.item_id === itemConstants_1.ITEMS.PILL_STAMINA_1 || item.item_id === itemConstants_1.ITEMS.PILL_STAMINA_2 || item.item_id === itemConstants_1.ITEMS.PILL_STAMINA_3) {
                ...
                database_1.default.transaction(() => {
                    UserRepository_1.userRepository.update(userId, { stamina: newStamina, y_canh: JSON.stringify(yCanh) });
                    InventoryRepository_1.inventoryRepository.removeItemById(inventoryId, 1);
                })();
                return {
                    success: true,
            ...
            if (item.type === 'chest') {
                ...
                database_1.default.transaction(() => {
                    ...
                    for (const r of rewards) {
                        InventoryRepository_1.inventoryRepository.addItem(userId, r.itemId, r.quantity, r.customStats);
                    }
                })();
                return {
                    success: true,
     ...
     */
    bindLifeArtifact(userId, inventoryId) {
        const item = InventoryRepository_1.inventoryRepository.get(inventoryId);
        if (!item || item.user_id !== userId) {
            return { success: false, message: 'Vật phẩm không tồn tại.' };
        }
        ...
        const existing = database_1.default.prepare('SELECT id, item_id FROM inventories WHERE user_id = ? AND is_life_bound = 1').get(userId);
        if (existing) {
            return { success: false, message: 'Đạo hữu chỉ có thể liên kết duy nhất 1 Bản Mệnh Pháp Bảo!' };
        }
        try {
            database_1.default.prepare('UPDATE inventories SET is_life_bound = 1, bound_level = 1, bound_exp = 0 WHERE id = ?').run(inventoryId);
    ...
    addArtifactExp(userId, exp) {
        ...
        try {
            if (leveledUp) {
                ...
                database_1.default.prepare('UPDATE inventories SET bound_level = ?, bound_exp = ?, custom_stats = ? WHERE id = ?')
                    .run(newLvl, newExp, JSON.stringify(customStats), boundItem.id);
            }
            else {
                database_1.default.prepare('UPDATE inventories SET bound_exp = ? WHERE id = ?').run(newExp, boundItem.id);
            }
        }
        catch (e) {
     ...
     */
    handleBreakthroughStats(userId) {
        const boundItem = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND is_life_bound = 1').get(userId);
        if (!boundItem)
        ...
        try {
            database_1.default.prepare('UPDATE inventories SET custom_stats = ? WHERE id = ?').run(JSON.stringify(customStats), boundItem.id);
            return {
                success: true,
     ...
     */
    swapLifeArtifact(userId, oldInvId, newInvId) {
        const oldItem = InventoryRepository_1.inventoryRepository.get(oldInvId);
        const newItem = InventoryRepository_1.inventoryRepository.get(newInvId);
        ...
        try {
            database_1.default.transaction(() => {
                // Hủy liên kết cũ
                database_1.default.prepare('UPDATE inventories SET is_life_bound = 0, bound_level = 1, bound_exp = 0 WHERE id = ?').run(oldInvId);
                // Thiết lập liên kết mới
                database_1.default.prepare('UPDATE inventories SET is_life_bound = 1, bound_level = ?, bound_exp = ? WHERE id = ?')
    ...
    openChests(username, chestId, qty) {
        const rewards = [];
        const summaryMap = new Map();
        const addReward = (itemId, name, quantity = 1, customStats = null) => {
    ...
    generateCustomStatsForChest(grade) {
        const stats = {};
        const lowerGrade = grade.toLowerCase();
        if (lowerGrade === 'f' || lowerGrade === 'd')
            return null;
        const rollStat = (type, min, max) => {
            return Math.floor(Math.random() * (max - min + 1)) + min;
        };
        const types = ['atk', 'def', 'hp', 'crit', 'luck'];
        let lines = 1;
        if (lowerGrade === 'c' || lowerGrade === 'b')
            lines = 1;
        else if (lowerGrade === 'a' || lowerGrade === 's')
            lines = 2;
        else if (lowerGrade === 'ss' || lowerGrade === 'sss')
            lines = 3;
        else if (lowerGrade === 'ex')
            lines = 4;
        ...
        for (const statType of chosenTypes) {
            if (statType === 'atk') {
                if (lowerGrade === 'c')
                    stats.atk = rollStat('atk', 2, 6);
                else if (lowerGrade === 'b')
                    stats.atk = rollStat('atk', 5, 15);
                else if (lowerGrade === 'a')
                    stats.atk = rollStat('atk', 10, 30);
                else if (lowerGrade === 's')
                    stats.atk = rollStat('atk', 20, 50);
                else if (lowerGrade === 'ss')
                    stats.atk = rollStat('atk', 40, 100);
                else if (lowerGrade === 'sss')
                    stats.atk = rollStat('atk', 80, 200);
                else if (lowerGrade === 'ex')
                    stats.atk = rollStat('atk', 150, 400);
            }
            else if (statType === 'def') {
                if (lowerGrade === 'c')
                    stats.def = rollStat('def', 1, 4);
                else if (lowerGrade === 'b')
                    stats.def = rollStat('def', 3, 10);
                else if (lowerGrade === 'a')
                    stats.def = rollStat('def', 6, 20);
                else if (lowerGrade === 's')
                    stats.def = rollStat('def', 12, 35);
                else if (lowerGrade === 'ss')
                    stats.def = rollStat('def', 25, 70);
                else if (lowerGrade === 'sss')
                    stats.def = rollStat('def', 50, 150);
                else if (lowerGrade === 'ex')
                    stats.def = rollStat('def', 100, 300);
            }
            else if (statType === 'hp') {
                if (lowerGrade === 'c')
                    stats.hp = rollStat('hp', 10, 30);
                else if (lowerGrade === 'b')
                    stats.hp = rollStat('hp', 25, 80);
                else if (lowerGrade === 'a')
                    stats.hp = rollStat('hp', 60, 200);
                else if (lowerGrade === 's')
                    stats.hp = rollStat('hp', 120, 400);
                else if (lowerGrade === 'ss')
                    stats.hp = rollStat('hp', 250, 800);
                else if (lowerGrade === 'sss')
                    stats.hp = rollStat('hp', 500, 1500);
                else if (lowerGrade === 'ex')
                    stats.hp = rollStat('hp', 1000, 3000);
            }
            else if (statType === 'crit') {
                let critVal = 0.01;
                if (lowerGrade === 'a')
                    critVal = 0.01 + Math.random() * 0.02;
                else if (lowerGrade === 's')
                    critVal = 0.02 + Math.random() * 0.03;
                else if (lowerGrade === 'ss')
                    critVal = 0.03 + Math.random() * 0.05;
                else if (lowerGrade === 'sss')
                    critVal = 0.05 + Math.random() * 0.07;
                else if (lowerGrade === 'ex')
                    critVal = 0.08 + Math.random() * 0.12;
                stats.crit = parseFloat(critVal.toFixed(3));
            }
            else if (statType === 'luck') {
                if (lowerGrade === 's')
                    stats.luck = rollStat('luck', 1, 3);
                else if (lowerGrade === 'ss')
                    stats.luck = rollStat('luck', 2, 6);
                else if (lowerGrade === 'sss')
                    stats.luck = rollStat('luck', 5, 15);
                else if (lowerGrade === 'ex')
                    stats.luck = rollStat('luck', 10, 30);
            }
        }
        return stats;
    }
    // === A3: Item Salvage System ===
    static SALVAGE_YIELDS = {
    ...
    salvageItem(userId, inventoryId) {
        const inv = database_1.default.prepare('SELECT * FROM inventories WHERE id = ? AND user_id = ?').get(inventoryId, userId);
        if (!inv)
            return { success: false, message: 'Vật phẩm không tồn tại trong hành trang!' };
        // Cannot salvage equipped items
        if (inv.equipped)
            return { success: false, message: 'Không thể phân loại vật phẩm đang mặc!' };
        const item = database_1.default.prepare('SELECT * FROM items WHERE id = ?').get(inv.item_id);
        if (!item)
            return { success: false, message: 'Không tìm thấy thông tin vật phẩm!' };
        ...
        // Remove the item
        if (inv.quantity > 1) {
            database_1.default.prepare('UPDATE inventories SET quantity = quantity - 1 WHERE id = ?').run(inventoryId);
        }
        else {
            database_1.default.prepare('DELETE FROM inventories WHERE id = ?').run(inventoryId);
        }
        // Add materials
        const existingMat = database_1.default.prepare('SELECT * FROM inventories WHERE user_id = ? AND item_id = ?').get(userId, salvage.materialId);
        if (existingMat) {
            database_1.default.prepare('UPDATE inventories SET quantity = quantity + ? WHERE id = ?').run(qty, existingMat.id);
        }
        else {
            database_1.default.prepare('INSERT INTO inventories (user_id, item_id, quantity) VALUES (?, ?, ?)').run(userId, salvage.materialId, qty);
        }
        const matItem = database_1.default.prepare('SELECT name FROM items WHERE id = ?').get(salvage.materialId);
