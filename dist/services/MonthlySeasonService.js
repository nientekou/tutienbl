...
exports.monthlySeasonService = void 0;
...
class MonthlySeasonService {
    ...
    getCurrentSeason() {
        ...
        let season = database_1.default.prepare('SELECT * FROM monthly_seasons WHERE month = ? AND year = ?').get(month, year);
        if (!season) {
      ...
      `).run(month, year, themeDef.theme, themeDef.element, themeDef.dungeon, themeDef.boss, themeDef.rewards, Math.floor(Date.now() / 1000));
            season = database_1.default.prepare('SELECT * FROM monthly_seasons WHERE month = ? AND year = ?').get(month, year);
        }
        return season;
    }
    /**
    ...
    updateProgress(userId, score) {
        this.initTable();
        const season = this.getCurrentSeason();
        const existing = database_1.default.prepare('SELECT * FROM user_season_progress WHERE user_id = ? AND month = ? AND year = ?')
            .get(userId, season.month, season.year);
        if (existing) {
            database_1.default.prepare('UPDATE user_season_progress SET score = score + ?, completed = completed + 1 WHERE user_id = ? AND month = ? AND year = ?')
                .run(score, userId, season.month, season.year);
        }
        else {
            database_1.default.prepare('INSERT INTO user_season_progress (user_id, month, year, score, completed) VALUES (?, ?, ?, ?, 1)')
    ...
    getSeasonLeaderboard(limit = 10) {
      ...
      LIMIT ?
    `).all(season.month, season.year, limit);
        return rows.map(r => ({
            userId: r.user_id,
            name: r.name,
            score: r.score,
...
exports.monthlySeasonService = new MonthlySeasonService();
