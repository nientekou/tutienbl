...
exports.infiniteDungeonService = void 0;
...
const INFINITE_EXCLUSIVE_REWARDS = [
...
class InfiniteDungeonService {
    ...
    `);
        ...
        try {
            database_1.default.exec(`ALTER TABLE infinite_dungeon_progress ADD COLUMN recent_modifiers TEXT DEFAULT '[]'`);
        }
    ...
    getProgress(userId) {
        this.initTable();
        let row = database_1.default.prepare('SELECT * FROM infinite_dungeon_progress WHERE user_id = ?').get(userId);
        if (!row) {
            database_1.default.prepare('INSERT INTO infinite_dungeon_progress (user_id, highest_floor, current_floor, recent_modifiers) VALUES (?, 0, 1, ?)')
                .run(userId, '[]');
            row = database_1.default.prepare('SELECT * FROM infinite_dungeon_progress WHERE user_id = ?').get(userId);
        }
        if (!row.recent_modifiers)
            row.recent_modifiers = '[]';
     ...
     */
    getFloorModifier(floor, recentModifiers = []) {
    ...
    challengeFloor(userId) {
        ...
        // Get player stats
        const playerStats = InventoryService_1.inventoryService.getActiveStats(userId);
        if (!playerStats)
            return { success: false, message: '❌ Lỗi tính toán chỉ số!' };
        // Get enemy
        const enemy = this.getEnemyStats(floor);
        const recentMods = JSON.parse(progress.recent_modifiers || '[]');
        ...
        const result = CombatEngine_1.CombatEngine.run(playerCombatant, enemyCombatant, null, 20);
      ...
      `).run(newHighest, floor + 1, result.totalDamageDealt, Math.floor(Date.now() / 1000), JSON.stringify(newRecentMods), userId);
            ...
            database_1.default.prepare('UPDATE infinite_dungeon_progress SET current_floor = ?, last_run_at = ? WHERE user_id = ?')
...
exports.infiniteDungeonService = new InfiniteDungeonService();
