        ...
        choices: [
            {
                ...
                successRate: 0.4,
                successReward: { item_id: itemConstants_1.ITEMS.TANG_BAO_DO, item_qty: 1, ngotinh: 10 },
        ...
        choices: [
            {
                ...
                successRate: 0.5,
                successReward: { item_id: itemConstants_1.ITEMS.TANG_BAO_DO, item_qty: 1, exp: 200 },
    ...
    },
    {
        ...
        choices: [
            ...
            },
            {
                ...
                successRate: 0.9,
                successReward: { item_id: itemConstants_1.ITEMS.POTION_STAMINA_WEEKLY, item_qty: 1 },
                failPenalty: {},
            },
        ],
    },
];
const SANYEUTHU_ENCOUNTERS = [
    {
        id: 'sy_giai_nhan',
        source: 'sanyeuthu',
        ...
        choices: [
            {
                ...
                text: '🔦 Khám phá cổ mộ',
                successRate: 0.5,
                successReward: { coins: 2000, exp: 300, item_id: itemConstants_1.ITEMS.MATERIAL_TINH_THIET_1, item_qty: 2 },
                failPenalty: { hp: -150, coins: -500 },
            },
            {
                id: 'sy_cm_ve',
        ...
        choices: [
            {
                ...
                text: '🧴 Lấy đan dược',
                successRate: 0.7,
                successReward: { coins: 500, exp: 150, item_id: itemConstants_1.ITEMS.PILL_TU_VI_LOW, item_qty: 3 },
        ...
        choices: [
            {
                ...
                successRate: 0.45,
                successReward: { item_id: itemConstants_1.ITEMS.TANG_BAO_DO, item_qty: 1, exp: 500 },
...
class EncounterService {
    ...
    resolveEncounter(userId, encounterId, choiceId) {
        ...
        if (!encounter)
            return { success: false, message: 'Sự kiện không tồn tại.', rewards: {} };
        const choice = encounter.choices.find(c => c.id === choiceId);
        if (!choice)
            return { success: false, message: 'Lựa chọn không hợp lệ.', rewards: {} };
    ...
    `).run(userId, encounterId, encounter.source, choiceId, isSuccess ? 'success' : 'fail', now);
        ...
        if (reward.contribution && reward.contribution !== 0 && user.sect_id) {
            ...
            database_1.default.prepare('UPDATE sects SET resources = resources + ? WHERE id = ?').run(Math.floor(reward.contribution / 2), user.sect_id);
        }
        ...
        if (reward.item_id && reward.item_qty) {
            InventoryRepository_1.inventoryRepository.addItem(userId, reward.item_id, reward.item_qty);
        ...
        if (reward.pet_template_id) {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0, 0, ?, ?, ?)
        `).run(userId, selected.name.split(' ')[0], selected.templateId, selected.rarity, selected.petBaseHp, selected.petBaseAtk, selected.petBaseDef, gender, nowTime, JSON.stringify(selected.skills));
                try {
...
exports.encounterService = new EncounterService();
