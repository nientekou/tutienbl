...
exports.abyssDungeonService = void 0;
...
class AbyssDungeonService {
    ...
    getProgress(userId) {
        ...
        let row = database_1.default.prepare('SELECT * FROM abyss_progress WHERE user_id = ?').get(userId);
        if (!row) {
            database_1.default.prepare('INSERT INTO abyss_progress (user_id, best_floor, current_floor, attempts_today, attempts_date) VALUES (?, 0, 0, 0, ?)')
                .run(userId, today);
            row = database_1.default.prepare('SELECT * FROM abyss_progress WHERE user_id = ?').get(userId);
        }
        if (row.attempts_date !== today) {
            database_1.default.prepare('UPDATE abyss_progress SET attempts_today = 0, attempts_date = ? WHERE user_id = ?')
                .run(today, userId);
    ...
    canEnter(userId, userLevel) {
        if (userLevel < 80)
            return { eligible: false, reason: 'Cần level 80+.' };
        const prog = this.getProgress(userId);
        if (prog.attemptsLeft <= 0)
            return { eligible: false, reason: 'Hết lượt hôm nay.' };
    ...
    recordFloor(userId, floor, won) {
        this.initTable();
        const today = this.getToday();
        if (!won) {
            database_1.default.prepare('UPDATE abyss_progress SET attempts_today = attempts_today + 1 WHERE user_id = ?').run(userId);
            const prog = this.getProgress(userId);
            return { pointsEarned: 0, nextFloor: prog.currentFloor, runEnd: true };
        }
        const nextFloor = floor + 1;
        const bestRow = database_1.default.prepare('SELECT best_floor FROM abyss_progress WHERE user_id = ?').get(userId);
    ...
    getLeaderboard(limit = 10) {
        this.initTable();
        const month = this.getCurrentMonth();
        return database_1.default.prepare('SELECT user_id as userId, best_floor as bestFloor FROM abyss_leaderboard WHERE month = ? ORDER BY best_floor DESC LIMIT ?').all(month, limit);
    }
...
exports.abyssDungeonService = new AbyssDungeonService();
