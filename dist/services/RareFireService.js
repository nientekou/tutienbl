...
class RareFireService {
     ...
     */
    fuseFires(userId, fireType1, fireType2) {
        const fire1 = database_1.default.prepare('SELECT * FROM rare_fires WHERE user_id = ? AND fire_type = ?').get(userId, fireType1);
        const fire2 = database_1.default.prepare('SELECT * FROM rare_fires WHERE user_id = ? AND fire_type = ?').get(userId, fireType2);
        if (!fire1 || !fire2)
            return { success: false, message: '❌ Cả 2 lửa phải tồn tại!' };
        if (fire1.id === fire2.id)
            return { success: false, message: '❌ Không thể hợp nhất chính mình!' };
        if (fire1.tier !== fire2.tier)
            return { success: false, message: '❌ 2 lửa phải cùng tier!' };
        if (fire1.tier >= 8)
            return { success: false, message: '❌ Đã đạt tier tối đa!' };
        // 70% success rate
        ...
        const def1 = rareFireConstants_1.RARE_FIRES.find(f => f.type === fireType1);
        const newTier = fire1.tier + 1;
        const newDef = rareFireConstants_1.RARE_FIRES.find(f => f.tier === newTier);
        database_1.default.transaction(() => {
            database_1.default.prepare('DELETE FROM rare_fires WHERE id = ?').run(fire1.id);
            database_1.default.prepare('DELETE FROM rare_fires WHERE id = ?').run(fire2.id);
            if (newDef) {
                database_1.default.prepare('INSERT INTO rare_fires (user_id, fire_type, fire_name, tier, level, exp) VALUES (?, ?, ?, ?, 1, 0)')
        ...
        return {
            success: true,
            message: `✨ **Hợp nhất thành công!** ${def1.name} + ${rareFireConstants_1.RARE_FIRES.find(f => f.type === fireType2)?.name} → **${newDef?.name || 'Không rõ'}** (Tier ${newTier})!`
        };
    }
    /**
     * W9-06: Weekly Fire Trial
     */
    enterFireTrial(userId) {
        const fire = database_1.default.prepare('SELECT * FROM rare_fires WHERE user_id = ? AND equipped = 1').get(userId);
        if (!fire)
            return { success: false, message: '❌ Cần equip 1 Hỏa Lửa để tham gia Trial!' };
        // Check weekly limit (3 trials/week)
        const weekKey = this.getWeekKey();
        const trialRow = database_1.default.prepare('SELECT * FROM fire_trials WHERE user_id = ? AND week_key = ?').get(userId, weekKey);
        ...
        // Update trial count
        if (trialRow) {
            database_1.default.prepare('UPDATE fire_trials SET trials_used = trials_used + 1 WHERE user_id = ? AND week_key = ?').run(userId, weekKey);
        }
        else {
            database_1.default.prepare('INSERT INTO fire_trials (user_id, week_key, trials_used) VALUES (?, ?, 1)').run(userId, weekKey);
        }
        if (isWin) {
     ...
     */
    awakenFire(userId, fireType) {
        const fire = database_1.default.prepare('SELECT * FROM rare_fires WHERE user_id = ? AND fire_type = ?').get(userId, fireType);
        if (!fire)
            return { success: false, message: '❌ Hỏa Lửa không tồn tại!' };
        if (fire.level < 100)
            return { success: false, message: `❌ Cần level 100 (hiện ${fire.level}).` };
        database_1.default.prepare('UPDATE rare_fires SET level = 1, exp = 0 WHERE id = ?').run(fire.id);
        CacheService_1.cacheService.invalidatePrefix(`rarefire:${userId}`);
        return {
            success: true,
     ...
     */
    getFireDescription(userId) {
        const fires = this.getUserFires(userId);
        const equipped = fires.find((f) => f.equipped === 1);
        let msg = `🔥 **Bộ Sưu Tập Hỏa Lửa**\n`;
        msg += `📊 Tổng cộng: **${fires.length}**\n`;
        if (equipped) {
            const def = rareFireConstants_1.RARE_FIRES.find((f) => f.type === equipped.fire_type);
