...
exports.mapFragmentService = void 0;
...
class MapFragmentService {
    ...
    getLocationById(locationId) {
        return database_1.default.prepare('SELECT * FROM treasure_locations WHERE id = ?').get(locationId);
    }
    stealLocation(userId, locationId) {
        const location = this.getLocationById(locationId);
        if (!location)
            return { success: false, message: '❌ Không tìm thấy vị trí kho báu này.' };
        if (location.owner_id === userId)
            return { success: false, message: '❌ Đây là kho báu của bạn! Không thể cướp của chính mình.' };
        if (location.is_claimed)
            return { success: false, message: '❌ Kho báu này đã bị khai thác.' };
        const now = Math.floor(Date.now() / 1000);
        if (location.expires_at <= now)
            return { success: false, message: '❌ Kho báu đã hết hạn (24h).' };
        ...
        database_1.default.prepare('UPDATE treasure_locations SET owner_id = ?, created_at = ?, expires_at = ? WHERE id = ?')
    ...
    claimLocation(userId, locationId) {
        const location = this.getLocationById(locationId);
        if (!location)
            return { success: false, message: '❌ Không tìm thấy vị trí kho báu này.' };
        if (location.owner_id !== userId)
            return { success: false, message: '❌ Đây không phải kho báu của bạn.' };
        if (location.is_claimed)
            return { success: false, message: '❌ Kho báu này đã được khai thác trước đó.' };
        const now = Math.floor(Date.now() / 1000);
        if (location.expires_at <= now)
            return { success: false, message: '❌ Kho báu đã hết hạn. Mảnh Bản Đồ đã mất đi linh khí.' };
        database_1.default.prepare('UPDATE treasure_locations SET is_claimed = 1 WHERE id = ?').run(locationId);
    ...
    grantRewards(userId, rarity) {
        ...
        switch (rarity) {
            case 'legendary': {
                ...
                UserRepository_1.userRepository.update(userId, { coin_ha_pham: (user.coin_ha_pham || 0) + ltAmount });
            ...
            case 'rare': {
                ...
                InventoryRepository_1.inventoryRepository.addItem(userId, rareMat, 2);
            ...
            default: {
                ...
                InventoryRepository_1.inventoryRepository.addItem(userId, commonMat, 3);
     ...
     */
    enhancedStealLocation(userId, locationId) {
        const user = require('../database/repositories/UserRepository').userRepository.get(userId);
        if (!user)
        ...
        if (location.is_claimed)
            return { success: false, message: '❌ Địa điểm đã được nhận!' };
        const now = Math.floor(Date.now() / 1000);
        if (location.expires_at < now)
            return { success: false, message: '❌ Địa điểm đã hết hạn!' };
        ...
        database_1.default.prepare('UPDATE treasure_locations SET owner_id = ?, expires_at = ? WHERE id = ?')
...
exports.mapFragmentService = new MapFragmentService();
