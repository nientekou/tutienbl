...
class HousingService {
    ...
    buyHouse(userId, houseType) {
        ...
        if (!house)
            return { success: false, message: 'Nhà không tồn tại' };
        if (user.coin_ha_pham < house.price) {
            return { success: false, message: `Không đủ Linh Thạch! Cần ${house.price}` };
        }
        UserRepository_1.userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham - house.price });
        database_1.default.prepare('INSERT OR REPLACE INTO user_houses (user_id, house_type) VALUES (?, ?)')
     ...
     * B-01: Buy furniture
     */
    buyFurniture(userId, furnitureId) {
        this.initTable();
        const user = UserRepository_1.userRepository.get(userId);
        if (!user)
            return { success: false, message: 'Người dùng không tồn tại' };
        ...
        if (!furniture)
            return { success: false, message: 'Nội thất không tồn tại' };
        if (user.coin_ha_pham < furniture.price) {
            return { success: false, message: `Không đủ Linh Thạch! Cần ${furniture.price}` };
        }
        UserRepository_1.userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham - furniture.price });
        // Add furniture to house
        const house = database_1.default.prepare('SELECT * FROM user_houses WHERE user_id = ?').get(userId);
        if (house) {
            const currentFurniture = JSON.parse(house.furniture || '[]');
            currentFurniture.push(furnitureId);
            database_1.default.prepare('UPDATE user_houses SET furniture = ? WHERE user_id = ?')
     ...
     */
    getHouseBonuses(userId) {
        this.initTable();
        const house = database_1.default.prepare('SELECT * FROM user_houses WHERE user_id = ?').get(userId);
        if (!house)
            return {};
        const furnitureIds = JSON.parse(house.furniture || '[]');
        const bonuses = {};
    ...
    getHousingDescription(userId) {
        ...
        if (!house) {
            ...
            const furnitureIds = JSON.parse(house.furniture || '[]');
            const decoIds = JSON.parse(house.decorations || '[]');
            ...
            if (furnitureIds.length > 0) {
                ...
                for (const fId of furnitureIds) {
                    const furniture = FURNITURE_LIST.find(f => f.id === fId);
                    if (furniture)
                        msg += `• ${furniture.name}: ${furniture.description}\n`;
                }
            }
            if (decoIds.length > 0) {
                msg += `\n**Trang trí:**\n`;
                for (const dId of decoIds) {
                    ...
                    if (deco)
                        msg += `• ${deco.name}: ${deco.description}\n`;
                }
            }
        }
        return msg;
    ...
    buyDecoration(userId, decoId) {
        ...
        if (!deco)
            return { success: false, message: 'Trang trí không tồn tại' };
        if (user.coin_ha_pham < deco.price) {
            return { success: false, message: `Không đủ Linh Thạch! Cần ${deco.price}` };
        }
        const house = database_1.default.prepare('SELECT * FROM user_houses WHERE user_id = ?').get(userId);
        if (!house)
            return { success: false, message: 'Hãy mua nhà trước!' };
        const decos = JSON.parse(house.decorations || '[]');
        if (decos.length >= 5)
            return { success: false, message: 'Ô trang trí đã đầy (tối đa 5)!' };
        if (decos.includes(decoId))
            return { success: false, message: 'Đã có trang trí này' };
        UserRepository_1.userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham - deco.price });
        decos.push(decoId);
        database_1.default.prepare('UPDATE user_houses SET decorations = ? WHERE user_id = ?')
            .run(JSON.stringify(decos), userId);
        return { success: true, message: `🎨 Mua trang trí: **${deco.name}**!` };
    }
    // === Visit Other Houses ===
    visitHouse(visitorId, ownerId) {
        this.initTable();
        const house = database_1.default.prepare('SELECT * FROM user_houses WHERE user_id = ?').get(ownerId);
        if (!house)
            return `🏠 Người dùng chưa có nhà.`;
        const owner = UserRepository_1.userRepository.get(ownerId);
        const houseType = HOUSE_TYPES.find(h => h.id === house?.house_type);
        const furnitureIds = JSON.parse(house.furniture || '[]');
        const decoIds = JSON.parse(house.decorations || '[]');
...
exports.housingService = new HousingService();
