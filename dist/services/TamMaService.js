...
class TamMaService {
     ...
     */
    getActiveDaoSkills(userId) {
        const rows = database_1.default.prepare('SELECT * FROM dao_comprehension WHERE user_id = ? AND level >= 3').all(userId);
        const daoSkillDefs = {
            sword_dao: {
     ...
     */
    getDaoDescription(userId) {
        const rows = database_1.default.prepare('SELECT * FROM dao_comprehension WHERE user_id = ?').all(userId);
        if (rows.length === 0)
            return 'Chưa có Dao nào được lĩnh ngộ.';
     ...
     */
    getCrossDaoSynergy(userId) {
        const rows = database_1.default.prepare('SELECT * FROM dao_comprehension WHERE user_id = ? AND level >= 2').all(userId);
        const synergies = [];
        const SYNERGY_MAP = {
     ...
     */
    buyFromDaoShop(userId, itemId) {
        const items = this.getDaoShopItems();
        const item = items.find(i => i.id === itemId);
        if (!item)
            return { success: false, message: 'Vật phẩm không tồn tại!' };
        const totalPoints = database_1.default.prepare('SELECT COALESCE(SUM(points), 0) as total FROM dao_comprehension WHERE user_id = ?')
            .get(userId);
        if (totalPoints.total < item.cost) {
            return { success: false, message: `Không đủ Điểm Đạo! (Cần ${item.cost}, có ${totalPoints.total})` };
        }
        database_1.default.prepare('UPDATE dao_comprehension SET points = points - ? WHERE user_id = ? AND points >= ?')
        ...
        if (item.type === 'title') {
            ...
            if (user) {
                UserRepository_1.userRepository.update(userId, { tu_vi: Math.min(user.tu_vi + 5000, user.exp_needed) });
            }
        }
