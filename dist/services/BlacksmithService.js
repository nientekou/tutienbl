...
exports.blacksmithService = exports.BlacksmithService = void 0;
...
class BlacksmithService {
     ...
     */
    getRecipes() {
        return Object.values(recipes_1.RECIPES).filter(r => r.type === 'forging');
    }
    /**
     * Thực hiện Rèn Khí
     */
    forgeItem(userId, recipeId) {
        ...
        if (recipe.minLevel > 30)
            baseSuccess = 0.65;
        if (recipe.minLevel > 60)
            baseSuccess = 0.60;
        let sectBonus = 0;
        if (user.sect_id) {
        ...
        try {
            ...
            eventBonus = eventService.getActiveBonus('craft_bonus');
        }
        catch { }
        ...
        // Trừ nguyên liệu
        for (const mat of recipe.ingredients) {
            InventoryRepository_1.inventoryRepository.removeItem(userId, mat.itemId, mat.quantity);
        }
        if (isSuccess) {
            ...
            let rewardName = '';
            const staticItem = database_1.default.prepare('SELECT name, stats FROM items WHERE id = ?').get(recipe.product.itemId);
     ...
     */
    dismantleItem(userId, inventoryIds) {
        if (inventoryIds.length === 0)
            return { success: false, message: 'Vui lòng chọn ít nhất 1 vật phẩm!' };
        ...
        `).get(invId, userId);
                if (!item || item.is_equipped === 1 || item.type !== 'equipment')
                    continue;
                let htGain = 0;
            ...
            if (totalHuyenThiet > 0) {
                ...
                const existing = database_1.default.prepare("SELECT id FROM inventories WHERE user_id = ? AND item_id = 'mat_huyen_thiet'").get(userId);
                if (existing) {
                    database_1.default.prepare('UPDATE inventories SET quantity = quantity + ? WHERE id = ?').run(totalHuyenThiet, existing.id);
                }
                else {
    ...
    `).get(inventoryId, userId);
        ...
        if (item.type !== 'equipment')
            return { success: false, message: 'Chỉ có thể tinh luyện trang bị!' };
        ...
        const tx = database_1.default.transaction(() => {
            UserRepository_1.userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham - ltNeeded });
            database_1.default.prepare('UPDATE inventories SET quantity = quantity - ? WHERE id = ?').run(htNeeded, htItem.id);
            if (isSuccess) {
                database_1.default.prepare('UPDATE inventories SET stars = stars + 1 WHERE id = ?').run(item.id);
            }
            else {
...
exports.BlacksmithService = BlacksmithService;
exports.blacksmithService = new BlacksmithService();
