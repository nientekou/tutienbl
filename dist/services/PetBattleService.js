...
exports.petBattleService = void 0;
const database_1 = __importDefault(require("../database/database"));
const PET_ELEMENT_COUNTERS = {
...
class PetBattleService {
     ...
     */
    getBattlePets(userId) {
        const pets = database_1.default.prepare('SELECT * FROM rare_beasts WHERE user_id = ? ORDER BY level DESC LIMIT 3').all(userId);
        return pets.map(p => ({
            id: p.id,
            name: p.beast_name,
    ...
    `).run(userId, won ? 1 : 0, won ? 0 : 1, won ? 10 : 2, now, won ? 1 : 0, won ? 0 : 1, won ? 10 : 2, now);
    ...
    getPetBattleDescription(userId) {
        ...
        if (pets.length > 0) {
            ...
            for (const p of pets) {
                const elementEmoji = { 'fire': '🔥', 'water': '💧', 'earth': '🪨', 'wind': '🌀', 'lightning': '⚡', 'wood': '🌿', 'metal': '⚔️' };
...
exports.petBattleService = new PetBattleService();
