...
exports.petMergeService = void 0;
...
class PetMergeService {
    ...
    mergePets(userId, pet1Id, pet2Id) {
        this.initTable();
        // Check prestige unlock
        if (!PrestigeService_1.prestigeService.hasPrestigeUnlock(userId, 'pet_merge_slot')) {
        ...
        // Get both pets
        const pet1 = database_1.default.prepare('SELECT * FROM mounts WHERE id = ? AND user_id = ?').get(pet1Id, userId);
        const pet2 = database_1.default.prepare('SELECT * FROM mounts WHERE id = ? AND user_id = ?').get(pet2Id, userId);
        if (!pet1 || !pet2) {
        ...
        const baseAtkByRarity = { common: 20, uncommon: 30, rare: 40, epic: 55, legendary: 70 };
        const baseDefByRarity = { common: 15, uncommon: 25, rare: 35, epic: 45, legendary: 60 };
        const baseHpByRarity = { common: 150, uncommon: 200, rare: 250, epic: 300, legendary: 400 };
        const getRarityIndex = (r) => ['common', 'uncommon', 'rare', 'epic', 'legendary'].indexOf(r);
        ...
        database_1.default.transaction(() => {
            UserRepository_1.userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham - MERGE_COST });
            database_1.default.prepare('DELETE FROM mounts WHERE id = ? AND user_id = ?').run(pet1Id, userId);
            database_1.default.prepare('DELETE FROM mounts WHERE id = ? AND user_id = ?').run(pet2Id, userId);
            database_1.default.prepare(`
        INSERT INTO mounts (user_id, name, template_id, rarity, level, exp, speed_bonus, stamina_save, is_tamed, skills, created_at)
        VALUES (?, ?, ?, ?, ?, 0, ?, 0, 1, '[]', ?)
      `).run(userId, mergedName, higherRarityPet.template_id, higherRarityPet.rarity, Math.round(avgLevel), Math.round(baseSpeedByRarity[rarityKey] * avgLevel * MERGE_STAT_MULT * 100) / 100, Math.floor(Date.now() / 1000));
            database_1.default.prepare(`
    ...
    getMergeHistory(userId) {
        this.initTable();
        const rows = database_1.default.prepare('SELECT * FROM user_pet_merges WHERE user_id = ? ORDER BY merged_at DESC LIMIT 10')
            .all(userId);
        return rows.map(r => ({
            pet1: `Pet #${r.pet1_id}`,
            pet2: `Pet #${r.pet2_id}`,
...
exports.petMergeService = new PetMergeService();
