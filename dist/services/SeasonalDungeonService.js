...
exports.seasonalDungeonService = void 0;
const database_1 = __importDefault(require("../database/database"));
const SEASONAL_DUNGEONS = [
...
class SeasonalDungeonService {
     ...
     */
    completeDungeon(userId) {
        this.initTable();
        const now = new Date();
        const vn = new Date(now.getTime() + 7 * 3600000);
        const month = vn.getUTCMonth() + 1;
        const year = vn.getUTCFullYear();
        const existing = database_1.default.prepare('SELECT * FROM seasonal_dungeon_progress WHERE user_id = ? AND month = ? AND year = ?')
...
exports.seasonalDungeonService = new SeasonalDungeonService();
