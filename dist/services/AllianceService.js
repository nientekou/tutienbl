...
class AllianceService {
    ...
    createAlliance(userId, name) {
        ...
        if (!user || !user.sect_id)
            return { success: false, message: '❌ Must be in a sect!' };
        // Check if already in alliance
        const existing = database_1.default.prepare('SELECT * FROM alliance_members WHERE sect_id = ?').get(user.sect_id);
        if (existing)
            return { success: false, message: '❌ Sect already in an alliance!' };
        const now = Math.floor(Date.now() / 1000);
        const result = database_1.default.prepare('INSERT INTO alliances (name, leader_sect_id, created_at) VALUES (?, ?, ?)')
            .run(name, user.sect_id, now);
        database_1.default.prepare('INSERT INTO alliance_members (alliance_id, sect_id, joined_at) VALUES (?, ?, ?)')
    ...
    joinAlliance(userId, allianceId) {
        ...
        if (!user || !user.sect_id)
            return { success: false, message: '❌ Must be in a sect!' };
        const alliance = database_1.default.prepare('SELECT * FROM alliances WHERE id = ? AND status = ?').get(allianceId, 'active');
        if (!alliance)
            return { success: false, message: '❌ Alliance not found!' };
        const existing = database_1.default.prepare('SELECT * FROM alliance_members WHERE sect_id = ?').get(user.sect_id);
        if (existing)
            return { success: false, message: '❌ Sect already in an alliance!' };
        const memberCount = database_1.default.prepare('SELECT COUNT(*) as c FROM alliance_members WHERE alliance_id = ?').get(allianceId);
        if (memberCount.c >= 5)
            return { success: false, message: '❌ Alliance full (max 5 sects)!' };
        database_1.default.prepare('INSERT INTO alliance_members (alliance_id, sect_id, joined_at) VALUES (?, ?, ?)')
    ...
    getAllianceInfo(allianceId) {
        this.initTable();
        const alliance = database_1.default.prepare('SELECT * FROM alliances WHERE id = ?').get(allianceId);
        if (!alliance)
            return null;
        const members = database_1.default.prepare('SELECT am.sect_id, s.name FROM alliance_members am JOIN sects s ON am.sect_id = s.id WHERE am.alliance_id = ?')
            .all(allianceId);
        return {
            name: alliance.name,
     ...
     */
    createAllianceWar(alliance1Id, alliance2Id) {
        this.initTable();
        const a1 = database_1.default.prepare('SELECT * FROM alliances WHERE id = ? AND status = ?').get(alliance1Id, 'active');
        const a2 = database_1.default.prepare('SELECT * FROM alliances WHERE id = ? AND status = ?').get(alliance2Id, 'active');
        if (!a1 || !a2)
            return { success: false, message: '❌ Alliance not found!' };
        const now = Math.floor(Date.now() / 1000);
        database_1.default.prepare('INSERT INTO alliance_wars (alliance1_id, alliance2_id, status, start_time, end_time, created_at) VALUES (?, ?, ?, ?, ?, ?)')
...
exports.allianceService = new AllianceService();
