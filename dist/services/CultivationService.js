...
class CultivationService {
    ...
    generateLinhCan() {
        ...
        if (rand < 15)
            elementCount = 1; // 15% single (was 5%)
        else if (rand < 65)
            elementCount = 2; // 50% dual (was 15%)
        else if (rand < 90)
            elementCount = 3; // 25% triple (was 30%)
        else
            elementCount = 4; // 10% quad (was 35% quad + 15% penta)
    ...
    getExpMultiplierForSource(playerLevel, sourceLevel) {
     ...
     */
    getCultivationSpeedMultiplier(linhCanJson) {
        try {
            const data = Object.keys(JSON.parse(linhCanJson)).length;
            if (data === 1)
                return 1.5; // Đơn Linh Căn: 1.5x
            if (data === 2)
                return 1.25; // Song Linh Căn: 1.25x
            if (data === 3)
                return 1.1; // Tam Linh Căn: 1.1x
            if (data === 4)
                return 1.0; // Tứ Linh Căn: 1.0x
     ...
     */
    claimIdleCultivation(discordId) {
        ...
        const { minorLevel } = (0, constants_1.getRealmDetails)(user.level);
        ...
        if (user.tu_vi >= user.exp_needed) {
            ...
            UserRepository_1.userRepository.update(discordId, { updated_at: now });
        ...
        if (user.sect_id) {
            try {
                const sect = database_1.default.prepare('SELECT tu_linh_level FROM sects WHERE id = ?').get(user.sect_id);
    ...
    practice(discordId) {
        ...
        if (user.alignment === 'demonic')
            deviationChance += 0.02;
        if (user.stamina < 100)
            deviationChance += 0.02;
        if (Math.random() < deviationChance) {
            const deviationEndTime = now + 900; // 15 phút
            UserRepository_1.userRepository.update(discordId, { qi_deviation_until: deviationEndTime });
            // Ghi log tẩu hỏa nhập ma
            database_1.default.prepare("INSERT INTO audit_logs (user_id, action, details, created_at) VALUES (?, 'qi_deviation', ?, ?)").run(discordId, JSON.stringify({ reason: 'practice' }), now);
            return {
                success: false,
        ...
        database_1.default.prepare("INSERT INTO audit_logs (user_id, action, details, created_at) VALUES (?, 'practice', ?, ?)").run(discordId, JSON.stringify({ gained }), Math.floor(Date.now() / 1000));
        const totalPractice = database_1.default.prepare("SELECT COUNT(*) as c FROM audit_logs WHERE user_id = ? AND action = 'practice'").get(discordId);
        const newlyUnlocked = [];
        newlyUnlocked.push(...AchievementService_1.achievementService.setProgress(discordId, 'tl_9', totalPractice.c));
        newlyUnlocked.push(...AchievementService_1.achievementService.setProgress(discordId, 'tl_10', totalPractice.c));
        newlyUnlocked.push(...AchievementService_1.achievementService.setProgress(discordId, 'tl_11', totalPractice.c));
        let achieveText = '';
        if (newlyUnlocked.length > 0) {
            const detailText = newlyUnlocked.map(a => {
                ...
                if (a.reward_coins > 0)
                    rewardDetails.push(`+${a.reward_coins} Linh Thạch 🟤`);
                if (a.reward_title)
     ...
     */
    breakthrough(discordId, usePill = false, forceSuccess = false, qiDeviationDisturbance = false) {
        const user = UserRepository_1.userRepository.get(discordId);
        if (!user) {
        ...
        if (!isMajor) {
            ...
            if (typeof usePill === 'string' && usePill === 'bequan') {
                ...
                UserRepository_1.userRepository.update(discordId, { coin_ha_pham: user.coin_ha_pham - cost });
                forceSuccess = true;
            ...
            if (rolled <= totalRate) {
                ...
                const leveledUp = AchievementService_1.achievementService.checkLevelAchievements(discordId, nextLevel);
                let achieveText = '';
                ...
                UserRepository_1.userRepository.update(discordId, updates);
                const updatedUser = UserRepository_1.userRepository.get(discordId);
    ...
    awakenYCanh(discordId) {
        ...
        // Trừ tài nguyên
        if (useNgoTinh) {
            UserRepository_1.userRepository.update(discordId, { ngotinh: user.ngotinh - 5 });
        }
        else {
            UserRepository_1.userRepository.update(discordId, { coin_ha_pham: user.coin_ha_pham - 500 });
        }
        ...
        if (currentLevel >= 10) {
            ...
            if (useNgoTinh) {
                UserRepository_1.userRepository.update(discordId, { ngotinh: user.ngotinh + 3 }); // Hoàn trả 3 ngộ tính
                message = `🌀 Đạo hữu ngộ ra **${rolled.name}**, nhưng Ý Cảnh này đã đạt **Cực Hạn (Cấp 10)**. Thiên địa trả lại **3** Điểm Ngộ Tính.`;
            }
            else {
                UserRepository_1.userRepository.update(discordId, { coin_ha_pham: user.coin_ha_pham + 300 }); // Hoàn trả 300 linh thạch
            ...
            UserRepository_1.userRepository.update(discordId, { y_canh: JSON.stringify(yCanhMap) });
            if (currentLevel === 0) {
        ...
        if (achieveId && newLevel > currentLevel) {
            const unlocked = AchievementService_1.achievementService.setProgress(discordId, achieveId, newLevel);
    ...
    reincarnate(discordId) {
        ...
        const newlyUnlocked = [];
        newlyUnlocked.push(...AchievementService_1.achievementService.updateProgress(discordId, 'tl_12', 1));
        newlyUnlocked.push(...AchievementService_1.achievementService.setProgress(discordId, 'tl_13', newLuanHoiCount));
        let achieveText = '';
        if (newlyUnlocked.length > 0) {
            const detailText = newlyUnlocked.map(a => {
                const rewardDetails = [];
     ...
     */
    temperLinhCan(discordId, targetElement) {
        const user = UserRepository_1.userRepository.get(discordId);
        if (!user) {
        ...
        database_1.default.transaction(() => {
            if (deductMethod === 'trung') {
                UserRepository_1.userRepository.update(discordId, { coin_trung_pham: user.coin_trung_pham - costTrung });
            }
            else {
                UserRepository_1.userRepository.update(discordId, { coin_ha_pham: user.coin_ha_pham - (costTrung * 100) });
            }
            // Cập nhật người chơi
            UserRepository_1.userRepository.update(discordId, {
...
exports.cultivationService = new CultivationService();
