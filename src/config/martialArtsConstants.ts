...
export type MartialArtElement = 'Hoa' | 'Thuy' | 'Moc' | 'Kim' | 'Tho' | 'Loi' | 'Phong';

export interface MartialArtDef {
  id: string;
  name: string;
  element: MartialArtElement;
  tier: 1 | 2 | 3;
  description: string;
...
export const ELEMENT_COUNTERS: Record<MartialArtElement, MartialArtElement> = {
  'Hoa': 'Kim', 'Kim': 'Moc', 'Moc': 'Tho', 'Tho': 'Thuy', 'Thuy': 'Hoa', 'Loi': 'Thuy', 'Phong': 'Loi',
};

export const ELEMENT_ADVANTAGE_BONUS = 0.25;
export const ELEMENT_DISADVANTAGE_PENALTY = 0.25;

export interface MartialArtFusion {
  id: string; name: string; ingredient1: string; ingredient2: string; result: string; successRate: number; description: string;
}

export const MARTIAL_ART_FUSIONS: MartialArtFusion[] = [
