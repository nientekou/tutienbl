...
try {
  ...
  // Dọn dẹp dữ liệu cũ
  db.prepare('DELETE FROM users WHERE discord_id IN (?, ?)').run(userIdA, userIdB);
  db.prepare('DELETE FROM farming_plots WHERE user_id IN (?, ?)').run(userIdA, userIdB);
  db.prepare('DELETE FROM sects WHERE master_id IN (?, ?)').run(userIdA, userIdB);
  db.prepare('DELETE FROM crafting_queues WHERE user_id IN (?, ?)').run(userIdA, userIdB);
  ...
  const harvestFail = farmingService.harvestPlot(userIdA, 0);
  ...
  const speedupRes = farmingService.speedupPlot(userIdA, 0);
  ...
  // Thu hoạch khi chín
  const harvestSuccess = farmingService.harvestPlot(userIdA, 0);
  console.log(`- Thu hoạch sau gia tốc: success = ${harvestSuccess.success}, tin nhắn = "${harvestSuccess.message}"`);

  const userInv = inventoryRepository.getUserInventory(userIdA);
  ...
  // Mở rộng ô đất
  userRepository.update(userIdA, { coin_ha_pham: 300 }); // cấp linh thạch
  console.log('Cấp 300 Linh thạch cho Trương Vô Kỵ.');
  const unlockRes = farmingService.unlockPlot(userIdA);
  ...
  userRepository.update(userIdA, { coin_ha_pham: 600 });
  console.log('Cấp 600 Linh thạch cho Trương Vô Kỵ.');

  // Sáng lập tông môn
  const sectRes = sectService.createSect(userIdA, 'Võ Đang Phái', 'Chân Võ Đãng Ma, Thái Cực Tự Nhiên.');
  console.log(`- Kết quả sáng lập Tông Môn: success = ${sectRes.success}, tin nhắn = "${sectRes.message}"`);
  
  const userASect = userRepository.get(userIdA)!;
  ...
  const disbandRes = sectService.leaveSect(userIdA);
  ...
  inventoryRepository.addItem(userIdA, 'material_linh_thao_1', 3);
  userRepository.update(userIdA, { coin_ha_pham: 50 });
  console.log('Cấp 3x Linh Thảo Hạ Phẩm và 50 Linh thạch cho Trương Vô Kỵ.');

  // Bắt đầu chế đan dược
  const craftRes = craftingService.startCrafting(userIdA, 'recipe_pill_tu_vi_low');
  console.log(`- Bắt đầu chế tạo Sơ Cấp Tụ Khí Đan: success = ${craftRes.success}, tin nhắn = "${craftRes.message}"`);

  let queue = craftingService.getQueue(userIdA);
  ...
  const claimFail = craftingService.claimCraftedItems(userIdA);
  ...
  db.prepare('UPDATE crafting_queues SET end_time = ? WHERE user_id = ?').run(tenMinsAgo, userIdA);

  queue = craftingService.getQueue(userIdA);
  ...
  const claimSuccess = craftingService.claimCraftedItems(userIdA);
  console.log(`- Thu lò thành công: success = ${claimSuccess.success}, tin nhắn = "${claimSuccess.message}"`);

  const userInvAfter = inventoryRepository.getUserInventory(userIdA);
