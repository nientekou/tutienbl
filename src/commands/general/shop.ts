...
export function getCategoryItems(categoryId: string): ShopItem[] {
  if (categoryId === 'dan') {
    return SHOP_ITEMS.filter(i => ['pill_hp_1', 'pill_hp_2', 'pill_stamina_1', 'pill_stamina_2', 'pill_stamina_3', 'potion_stamina_weekly', 'pill_tu_vi_low', 'pill_break_1', 'pill_break_minor_1', 'pill_break_minor_2', 'pill_break_minor_3'].includes(i.id));
  }
  if (categoryId === 'nguyenlieu') {
    return SHOP_ITEMS.filter(i => ['seed_linh_thao_1', 'seed_nhan_sam_1', 'seed_tuyet_lien', 'seed_lingzhi', 'seed_ngodong', 'cauldron_low', 'cauldron_mid', 'cauldron_high'].includes(i.id));
  }
  if (categoryId === 'bua') {
    return SHOP_ITEMS.filter(i => ['talisman_anti_loi', 'talisman_speed_1'].includes(i.id));
  }
  if (categoryId === 'dacbiet') {
    return SHOP_ITEMS.filter(i => ['lucky_chest', 'chest_1tr5', 'server_raid_chest', 'item_nhan_dinh_hon', 'item_tam_sinh_thach', 'item_tuyet_tinh_nuoc', 'item_nhan_dinh_hon_knb', 'item_bloodline_pill_knb'].includes(i.id));
  }
  return [];
}
