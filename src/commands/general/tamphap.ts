      ...
      new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
          sub.setName('mang')
            ...
            .addStringOption(opt =>
              opt.setName('tam_phap')
                .setDescription('Tâm Pháp muốn trang bị')
                .setRequired(true)
                .addChoices(
                  { name: '🔥 Hỏa Linh Quyết (Hỏa)', value: 'hl_hoa' },
                  { name: '❄️ Băng Tâm Quyết (Thủy)', value: 'hl_thuy' },
                  { name: '🌿 Tụ Linh Quyết (Mộc)', value: 'hl_moc' },
                  { name: '🪨 Trường Sinh Quyết (Thổ)', value: 'hl_tho' },
                  { name: '⚡ Thần Hành Quyết (Kim)', value: 'hl_kim' },
                  { name: '🌀 Phá Cấm Quyết (Vô)', value: 'hl_vo' }
                )
            )
            .addIntegerOption(opt =>
              opt.setName('o_trang_bi')
          ...
          sub.setName('linhngo')
            ...
            .addStringOption(opt =>
              opt.setName('tam_phap')
                ...
                .setRequired(true)
                .addChoices(
                  { name: '🔥 Hỏa Linh Quyết', value: 'hl_hoa' },
                  { name: '❄️ Băng Tâm Quyết', value: 'hl_thuy' },
                  { name: '🌿 Tụ Linh Quyết', value: 'hl_moc' },
                  { name: '🪨 Trường Sinh Quyết', value: 'hl_tho' },
                  { name: '⚡ Thần Hành Quyết', value: 'hl_kim' },
                  { name: '🌀 Phá Cấm Quyết', value: 'hl_vo' }
                )
            )
        )
        .addSubcommand(sub =>
  ...
  public async execute(client: TuTienClient, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (sub === 'danhsach') {
      ...
      for (const law of laws) {
        ...
        let fragGoal = 5;
        if (law.level > 0) fragGoal = law.level * 5; // Chi phí nâng cấp tiếp theo
    ...
    // ───────────────── TRANG BỊ TÂM PHÁP ĐANG MANG ─────────────────
      ...
      const equipped = heartLawService.getEquippedHeartLaws(userId);
      const activePassives = heartLawService.getActivePassives(userId);
