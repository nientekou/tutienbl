...
// A5: Prestige Milestone Unlocks
interface PrestigeUnlock {
  id: string;
  name: string;
  description: string;
...
const PRESTIGE_UNLOCKS: PrestigeUnlock[] = [
...
interface PrestigeData {
  ...
  total_prestige_time: number;
  prestige_unlocks: string; // JSON array of unlock IDs
}
...
interface PrestigeShopItem {
  ...
  description: string;
  cost: number;
  type: 'cosmetic' | 'convenience' | 'title' | 'permanent' | 'consumable';
  emoji: string;
}

const PRESTIGE_SHOP_ITEMS: PrestigeShopItem[] = [
...
class PrestigeService {
  private initTable(): void {
    ...
    try {
      db.exec(`ALTER TABLE prestige_data ADD COLUMN prestige_unlocks TEXT DEFAULT '[]'`);
   ...
   */
  getPrestigeData(userId: string): PrestigeData {
    this.initTable();
    let row = db.prepare('SELECT * FROM prestige_data WHERE user_id = ?').get(userId) as PrestigeData | undefined;

    if (!row) {
      db.prepare('INSERT INTO prestige_data (user_id, prestige_level, prestige_tokens, total_prestige_time) VALUES (?, 0, 0, 0)')
        .run(userId);
      row = db.prepare('SELECT * FROM prestige_data WHERE user_id = ?').get(userId) as PrestigeData;
    }
   ...
   */
  canPrestige(userId: string): { can: boolean; reason: string } {
    const user = userRepository.get(userId);
    if (!user) return { can: false, reason: 'Chưa tạo nhân vật!' };
   ...
   */
  prestige(userId: string): { success: boolean; message: string } {
    const { can, reason } = this.canPrestige(userId);
    if (!can) return { success: false, message: `❌ ${reason}` };
    ...
    if (newUnlocks.length > 0) {
      const existingUnlocks: string[] = JSON.parse(prestige.prestige_unlocks || '[]');
      for (const unlock of newUnlocks) {
        if (!existingUnlocks.includes(unlock.id)) {
          existingUnlocks.push(unlock.id);
        }
      }
      db.prepare('UPDATE prestige_data SET prestige_unlocks = ? WHERE user_id = ?')
   ...
   */
  getPrestigeBonuses(userId: string): { atk: number; def: number; hp: number; speed: number } {
    const prestige = this.getPrestigeData(userId);
   ...
   */
  buyFromShop(userId: string, itemId: string): { success: boolean; message: string } {
    ...
    db.prepare('UPDATE prestige_data SET prestige_tokens = prestige_tokens - ? WHERE user_id = ?')
   ...
   */
  getPrestigeDescription(userId: string): string {
    const prestige = this.getPrestigeData(userId);
  ...
  hasPrestigeUnlock(userId: string, unlockId: string): boolean {
    const prestige = this.getPrestigeData(userId);
    const unlocks: string[] = JSON.parse(prestige.prestige_unlocks || '[]');
    return unlocks.includes(unlockId);
  }

  getPrestigeUnlocks(userId: string): { unlocked: PrestigeUnlock[]; locked: PrestigeUnlock[] } {
    const prestige = this.getPrestigeData(userId);
    const unlocks: string[] = JSON.parse(prestige.prestige_unlocks || '[]');
  ...
  getUnlocksDescription(userId: string): string {
   ...
   */
  getLeaderboard(limit: number = 10): { userId: string; name: string; prestigeLevel: number; tokens: number }[] {
    this.initTable();
