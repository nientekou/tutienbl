...
export class CombatService {
  ...
  public challengeDungeon(
    ...
    if (petRaw) {
      ...
      try { mutations = JSON.parse(petRaw.mutations || '{}'); } catch(e) { console.warn('[CombatService] Failed to parse pet mutations:', e); }
    ...
    if (combatResult.winner === 'player') {
      ...
      const lootEventBonus = eventService.getActiveBonus('loot_bonus');
      ...
      for (const loot of dungeon.rewards.loots) {
        ...
        if (lootEventBonus > 0) activeRate *= (1 + lootEventBonus);
      ...
      if (difficulty === 'ác_mộng' && Math.random() < 0.15) {
        ...
        const targetPhoiId = isWeapon ? ITEMS.PHOI_WEAPON_SSS : ITEMS.PHOI_ARMOR_SSS;
      ...
      const soulDropRate: Record<string, number> = { dễ: 0.10, thường: 0.20, khó: 0.35, ác_mộng: 0.50 };
      let soulDropChance = soulDropRate[difficulty] || 0.10;
      if (lootEventBonus > 0) soulDropChance *= (1 + lootEventBonus);
      if (Math.random() < soulDropChance) {
        ...
        const soulItem = db.prepare('SELECT name FROM items WHERE id = ?').get(chosen.id) as { name: string } | undefined;
        if (soulItem) loots.push({ id: chosen.id, name: `💀 ${soulItem.name}`, quantity: 1 });
      }
  ...
  public challengeWorldBoss(userId: string, clearCooldownWithCoin: boolean = false): BossChallengeResult {
    ...
    const pveScale = autoBalanceService.getPvEScaleFactor(userId);
   ...
   * Hệ thống mới: milestone + soft cap + sqrt + catch-up + boss point + lucky reward
   */
  public distributeWorldBossRewards(bossLevel: number, finalBlowerId: string): string[] {
    ...
    const factor = Math.min(bossLevel, 20);
    const totalDamage = participants.reduce((sum, p) => sum + p.damage, 0);
    if (totalDamage === 0 || participants.length === 0) return rewardsLogs;

    // Tính sức mạnh trung bình server để catch-up
    let serverMedianPower = 500;
    try {
      const allUsers = db.prepare('SELECT base_hp, base_atk, base_def FROM users ORDER BY RANDOM() LIMIT 200').all() as any[];
      if (allUsers.length > 0) {
        const powers = allUsers.map(u => (u.base_hp || 100) * 0.2 + (u.base_atk || 15) * 3 + (u.base_def || 10) * 5);
        powers.sort((a, b) => a - b);
        serverMedianPower = powers[Math.floor(powers.length / 2)] || 500;
      }
    } catch (e) {}

    // Lấy season_id hiện tại
    let currentSeasonId: number | null = null;
    try {
      const season = db.prepare('SELECT id FROM boss_seasons ORDER BY id DESC LIMIT 1').get() as any;
    ...
    const softCapFactor = (dmgPercent: number): number => {
      ...
      if (dmgPercent <= 0.30) return 0.70 - (dmgPercent - 0.20) / 0.10 * 0.20;
    ...
    for (let i = 0; i < participants.length; i++) {
      ...
      const cappedPercent = dmgPercent * softCapFactor(dmgPercent);
      ...
      const seasonalExpBonus = eventCalendarService.getSeasonalEffect('exp_bonus');
      const seasonalCoinBonus = eventCalendarService.getSeasonalEffect('drop_bonus');
      ...
      if (Math.random() < luckyChance) {
        ...
        if (luckyRoll < 0.05) {
          itemsToAdd.push({ userId: p.user_id, itemId: ITEMS.SERVER_RAID_CHEST, quantity: 1 });
          itemsGained.push('🎊 1x Rương Boss Thế Giới (Lucky!)');
      ...
      // Cập nhật tu vi, coin, boss points, KNB
      try {
        ...
        userRepository.update(p.user_id, {
          ...
          boss_points: (pUser as any).boss_points + gainedBossPoints,
      ...
      // Ghi log boss kill cho season
      if (currentSeasonId) {
        try {
          db.prepare(`INSERT INTO boss_kill_log (boss_name, boss_level, killed_at, total_damage, participant_count, final_blower, season_id) 
            SELECT name, level, ?, ?, ?, ?, ? FROM world_boss WHERE id = 'world_boss_current'`)
            .run(Math.floor(Date.now() / 1000), totalDamage, participants.length, finalBlowerId, currentSeasonId);
        } catch (e) {}
      }

      const itemsText = itemsGained.length > 0 ? ` + 🎁 [${itemsGained.join(', ')}]` : '';
      const knbText = gainedKnb > 0 ? `, +**${gainedKnb}** KNB` : '';
      const bpText = gainedBossPoints > 0 ? `, +**${gainedBossPoints}** BP` : '';
      rewardsLogs.push(
        `🏅 **Hạng ${i + 1}**: **${pUser.name}** (${p.damage} dmg, ${(dmgPercent * 100).toFixed(1)}%) → +${gainedExp} Tu Vi, +${gainedCoins} LT${bpText}${knbText}${itemsText}`
      );
    }
   ...
   */
  public logBossAttack(userId: string, damage: number, skill: string, isCrit: boolean, bossHpPercent: number): void {
    try {
      db.prepare(`INSERT INTO boss_attack_log (user_id, damage, skill, is_crit, boss_hp_percent, created_at) VALUES (?, ?, ?, ?, ?, ?)`)
        .run(userId, damage, skill, isCrit ? 1 : 0, Math.round(bossHpPercent * 100) / 100, Math.floor(Date.now() / 1000));
      // Giữ chỉ 20 log gần nhất
      db.prepare(`DELETE FROM boss_attack_log WHERE id NOT IN (SELECT id FROM boss_attack_log ORDER BY created_at DESC LIMIT 20)`).run();
    } catch (e) {}
  }
   ...
   */
  public getRecentBossAttacks(limit: number = 5): Array<{ name: string; damage: number; skill: string; isCrit: boolean; hpPercent: number }> {
