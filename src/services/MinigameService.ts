...
const WEEKLY_PRIZES = [
  { // 🥇 Hạng 1
    ...
    items: [{ item_id: ITEMS.REPAIR_STONE_MID, quantity: 3 }]
  },
  { // 🥈 Hạng 2
    coins: 3000, tuVi: 1000, ngotinh: 5,
    items: [{ item_id: ITEMS.REPAIR_STONE_MID, quantity: 2 }]
  },
  { // 🥉 Hạng 3
...
/** Hành động quyết đấu mới (6 chiêu thức) */
export type DuelAction = 'xuất_kiếm' | 'phòng_thủ' | 'linh_pháp' | 'tụ_khí' | 'tuyệt_kỹ' | 'dùng_vật_phẩm';
...
export interface DuelState {
  ...
  targetId: string;
  wager: number;
  challengerChoice?: DuelAction;
  targetChoice?: DuelAction;
  createdAt: number;
  acceptedAt?: number;
  status: 'pending' | 'accepted' | 'refused' | 'completed' | 'expired';
  /** Số hiệp hiện tại */
  currentRound: number;
  /** Tổng số hiệp (mặc định 3) */
  maxRounds: number;
  /** Máu chiến đấu của người thách */
  challengerHp: number;
  challengerMaxHp: number;
  /** Máu chiến đấu của người bị thách */
  targetHp: number;
  targetMaxHp: number;
  /** Buff tăng sát thương hiệp sau */
  challengerBuffed: boolean;
  targetBuffed: boolean;
  /** Theo dõi đã sử dụng Tuyệt Kỹ chưa (1 lần/trận) */
  challengerUltUsed: boolean;
  targetUltUsed: boolean;
  /** Theo dõi id túi đồ vật phẩm được sử dụng trong hiệp (nếu có) */
  challengerUsedItemId?: string;
  targetUsedItemId?: string;
  /** Trạng thái hiệu ứng kỹ năng môn phái */
  challengerEffects: DuelSkillEffects;
  targetEffects: DuelSkillEffects;
  /** Nhật ký các hiệp */
  roundLogs: string[];
  roundStatus: 'waiting' | 'resolving' | 'round_complete' | 'complete';
}

/** Hiệu ứng kỹ năng trong quyết đấu */
export interface DuelSkillEffects {
  burn?: { remaining: number; damagePerTurn: number };
  shield?: number;
  stun?: boolean;
  dodge?: boolean;
  lifesteal?: boolean;
...
export const DUEL_SKILL_DEFS: Record<string, {
  name: string;
  element: string;
  emoji: string;
  description: string;
...
export const DUEL_ACTION_META: Record<DuelAction, { emoji: string; name: string; desc: string }> = {
...
const DUEL_DAMAGE_MATRIX: Record<DuelAction, Record<DuelAction, number>> = {
...
if (!globalAny.__activeDuels) {
  globalAny.__activeDuels = new Map<string, DuelState>();
}

export class MinigameService {
  private activeDuels: Map<string, DuelState> = globalAny.__activeDuels;
   ...
   */
  public drawFortuneStick(userId: string): {
    success: boolean;
    message: string;
    title?: string;
    outcome?: 'daicat' | 'trungcat' | 'tieucat' | 'binhthuong' | 'hung' | 'daihung';
    coinDiff?: number;
    luckDiff?: number;
    ...
    let outcome: 'daicat' | 'trungcat' | 'tieucat' | 'binhthuong' | 'hung' | 'daihung';
    let title = '';
    let description = '';
    let coinDiff = 0;
    let luckDiff = 0;
    ...
    if (updatedBaseLuck > 30) updatedBaseLuck = 30;
    if (updatedBaseLuck < 5) updatedBaseLuck = 5;
   ...
   */
  public createChallenge(challengerId: string, targetId: string, wager: number): {
    success: boolean;
    message: string;
    duel?: DuelState;
  } {
    ...
    const chillerStats = inventoryService.getActiveStats(challengerId);
    const targetStats = inventoryService.getActiveStats(targetId);
    ...
    const scaleHp = (hp: number) => Math.max(300, Math.round(hp * 0.5));
   ...
   */
  public getDuel(duelId: string): DuelState | undefined {
   ...
   */
  public acceptChallenge(duelId: string, userId: string): {
    success: boolean;
    message: string;
    duel?: DuelState;
  } {
    ...
    const challenger = userRepository.get(duel.challengerId);
    const target = userRepository.get(duel.targetId);
   ...
   */
  public refuseChallenge(duelId: string, userId: string): {
    success: boolean;
    message: string;
    duel?: DuelState;
  } {
   ...
   */
  public chooseMove(duelId: string, userId: string, choiceRaw: string): {
    success: boolean;
    message: string;
    duel?: DuelState;
    winnerId?: string;
    ...
    let choice = choiceRaw as DuelAction;
    let itemIdToUse: string | undefined = undefined;
    ...
    if (userId === duel.challengerId) {
      ...
      if (choice === 'tuyệt_kỹ') duel.challengerUltUsed = true;
      if (choice === 'dùng_vật_phẩm') duel.challengerUsedItemId = itemIdToUse;
      ...
      if (choice === 'tuyệt_kỹ') duel.targetUltUsed = true;
      if (choice === 'dùng_vật_phẩm') duel.targetUsedItemId = itemIdToUse;
    }
   ...
   */
  private getEquippedSkills(userId: string): Array<{ skill_id: string; level: number; equipped_slot: number }> {
    try {
      return db.prepare(
        'SELECT skill_id, level, equipped_slot FROM user_skills WHERE user_id = ? AND is_equipped = 1 ORDER BY equipped_slot ASC'
      ).all(userId) as Array<{ skill_id: string; level: number; equipped_slot: number }>;
    } catch {
      return [];
    }
  }
   ...
   */
  private getPrimaryElement(userId: string): string {
    try {
      const user = userRepository.get(userId);
      if (!user?.linh_can) return 'Vô thuộc';
      const linhCan = JSON.parse(user.linh_can);
      const keys = Object.keys(linhCan);
      if (keys.length === 0) return 'Vô thuộc';
   ...
   */
  private resolveDuelRound(duel: DuelState): { challengerDmg: number; targetDmg: number; challengerHeal: number; targetHeal: number; narrative: string } {
    ...
    let chillerSkillActivated: string | null = null;
    let targetSkillActivated: string | null = null;
    ...
    if (challengerAction === 'tuyệt_kỹ') {
      ...
      if (mainSkill && DUEL_SKILL_DEFS[mainSkill.skill_id]) {
        chillerSkillActivated = mainSkill.skill_id;
        chillerElementMatch = DUEL_SKILL_DEFS[mainSkill.skill_id].element === chillerElement;
      }
    } else {
      for (const skill of chillerSkills) {
        const def = DUEL_SKILL_DEFS[skill.skill_id];
        if (!def) continue;
    ...
    if (targetAction === 'tuyệt_kỹ') {
      ...
      if (mainSkill && DUEL_SKILL_DEFS[mainSkill.skill_id]) {
        ...
        targetElementMatch = DUEL_SKILL_DEFS[mainSkill.skill_id].element === targetElement;
      }
    } else {
    ...
    if (duel.challengerEffects.burn && duel.challengerEffects.burn.remaining > 0) {
      ...
      if (duel.challengerEffects.burn.remaining <= 0) delete duel.challengerEffects.burn;
    }
    if (duel.targetEffects.burn && duel.targetEffects.burn.remaining > 0) {
      ...
      if (duel.targetEffects.burn.remaining <= 0) delete duel.targetEffects.burn;
    }
    ...
    let itemNarrative = '';
    const applyItem = (userId: string, usedItemId: string | undefined, isChallenger: boolean) => {
      if (!usedItemId) return;
      const invItem = inventoryRepository.get(parseInt(usedItemId, 10));
      if (invItem && invItem.user_id === userId) {
        inventoryRepository.removeItemById(invItem.id, 1);
        try {
          // Lấy stats mặc định nếu custom_stats không có
          const dbItems = require('../database/database').ITEMS || [];
          const itemDef = dbItems.find((i: any) => i.id === invItem.item_id);
          const stats = JSON.parse(invItem.custom_stats || itemDef?.stats || '{}');
          
          let heal = 0;
          if (stats.restore_hp) heal = stats.restore_hp;
          else if (stats.add_tu_vi) heal = 50; // Tụ khí đan cũng hồi máu nhẹ trong PvP
          else if (stats.add_stamina) heal = 30;
        ...
        } catch (e) { console.warn('[MinigameService] Failed to use HP pill in duel:', e); }
      }
    };
    ...
    if (DUEL_DAMAGE_MATRIX[challengerAction][targetAction] > 0) {
      ...
      if (duel.challengerBuffed) multiplier *= 1.25;
      // Lôi Phạt Quyết: +50% damage
      if (chillerSkillActivated === 'skill_lightning') multiplier *= chillerDmgBuff;
    ...
    if (DUEL_DAMAGE_MATRIX[targetAction][challengerAction] > 0) {
      ...
      if (duel.targetBuffed) multiplier *= 1.25;
      if (targetSkillActivated === 'skill_lightning') multiplier *= targetDmgBuff;
    ...
    if (duel.targetEffects.shield && duel.targetEffects.shield > 0 && challengerDmg > 0) {
      ...
      if (challengerDmg < 0) challengerDmg = 0;
    }
    ...
    const chillerPet = db.prepare('SELECT name, base_atk, base_def, base_hp, rarity FROM pets WHERE user_id = ? AND is_deployed = 1').get(duel.challengerId) as any;
    const targetPet = db.prepare('SELECT name, base_atk, base_def, base_hp, rarity FROM pets WHERE user_id = ? AND is_deployed = 1').get(duel.targetId) as any;

    const processPet = (pet: any, isChallenger: boolean) => {
      ...
      if (Math.random() < 0.35) {
        ...
        if (petRoll < 0.5) {
          ...
          if (isChallenger) { challengerDmg += bonusDmg; } else { targetDmg += bonusDmg; }
          ...
          if (isChallenger) {
            ...
            if (actual > 0) petNarrative += `🐾 Sủng thú **${pet.name}** đỡ đòn thay chủ, hấp thụ **-${actual}** sát thương!\n`;
          } else {
          ...
          if (isChallenger) { challengerHeal += healAmount; } else { targetHeal += healAmount; }
          petNarrative += `🐾 Sủng thú **${pet.name}** vận chuyển linh lực chữa trị, hồi **+${healAmount}** HP!\n`;
        }
      }
    };

    if (chillerPet) processPet(chillerPet, true);
    if (targetPet) processPet(targetPet, false);
    ...
    // === TẠO TƯỜNG THUẬT ===
    const cMeta = DUEL_ACTION_META[challengerAction];
    const tMeta = DUEL_ACTION_META[targetAction];
    ...
    if (burnNarrative) narrative += burnNarrative + '\n';
    ...
    if (challengerAction === 'xuất_kiếm') {
      ...
      const chiller = userRepository.get(duel.challengerId);
      ...
      const skillName = chillerSkillActivated ? DUEL_SKILL_DEFS[chillerSkillActivated].name : 'Tuyệt Kỹ';
    ...
    if (targetAction === 'xuất_kiếm') {
      ...
      const skillName = targetSkillActivated ? DUEL_SKILL_DEFS[targetSkillActivated].name : 'Tuyệt Kỹ';
    ...
    const statusIcons = (effects: DuelSkillEffects) => {
      let icons = '';
      if (effects.burn && effects.burn.remaining > 0) icons += ` 🔥[${effects.burn.remaining}]`;
      if (effects.shield && effects.shield > 0) icons += ` 🪨[${effects.shield}]`;
      if (effects.dodge) icons += ` 🌀`;
      return icons;
    };

    narrative += `\n📊 **Trạng thái hiện tại:**\n`;
    narrative += `<@${duel.challengerId}>: ${hpBar(duel.challengerHp, duel.challengerMaxHp)} **${duel.challengerHp}/${duel.challengerMaxHp}** HP`;
    if (duel.challengerBuffed) narrative += ` 💫 [Chiến Ý]`;
    ...
    if (duel.targetBuffed) narrative += ` 💫 [Chiến Ý]`;
   ...
   */
  private finalizeDuel(duel: DuelState): {
    success: boolean;
    message: string;
    duel?: DuelState;
    winnerId?: string;
   ...
   */
  private executeDuelPaymentTransaction(winnerId: string, loserId: string, wager: number, winnings: number): void {
    const transaction = db.transaction((winnerId: string, loserId: string, wager: number, winnings: number) => {
      const loser = db.prepare('SELECT coin_ha_pham FROM users WHERE discord_id = ?').get(loserId) as { coin_ha_pham: number };
      const winner = db.prepare('SELECT coin_ha_pham FROM users WHERE discord_id = ?').get(winnerId) as { coin_ha_pham: number };
      ...
      // Khấu trừ bên bại
      db.prepare('UPDATE users SET coin_ha_pham = coin_ha_pham - ? WHERE discord_id = ?').run(wager, loserId);
      // Kết chuyển bên thắng (nhận net: +winnings - wager)
      db.prepare('UPDATE users SET coin_ha_pham = coin_ha_pham + ? WHERE discord_id = ?').run(winnings - wager, winnerId);
    });
   ...
   */
  private saveDuelHistory(duel: DuelState, winnerId: string | null, loserId: string | null, isTie: boolean, actualWinnings?: number, actualTax?: number): void {
    try {
      const now = Math.floor(Date.now() / 1000);
      ...
      const winner = isTie ? userRepository.get(duel.challengerId) : (winnerId ? userRepository.get(winnerId) : null);
      const loser = isTie ? userRepository.get(duel.targetId) : (loserId ? userRepository.get(loserId) : null);
   ...
   */
  public getDuelStats(userId: string): {
    totalMatches: number;
    wins: number;
    losses: number;
    ties: number;
    ...
    mostFrequentOpponent: { id: string; name: string; count: number } | null;
    ...
    `).all(userId, userId) as Array<{ winner_id: string; is_tie: number; fought_at: number }>;
    ...
    const opponentData = db.prepare(`
      ...
      ORDER BY cnt DESC
      LIMIT 1
    `).get(userId, userId, userId, userId) as { opponent_id: string; opponent_name: string; cnt: number } | undefined;
   ...
   */
  public getDuelHistory(userId: string, page: number = 1, pageSize: number = 5): {
    records: any[];
    totalRecords: number;
    totalPages: number;
    currentPage: number;
   ...
   */
  public getDuelLeaderboard(limit: number = 10, currentUserId?: string): {
    entries: Array<{
      userId: string;
      name: string;
      wins: number;
    ...
    const winsRows = db.prepare(`
      ...
      WHERE is_tie = 0
      GROUP BY winner_id
    `).all() as Array<{ winner_id: string; wins: number; net_earned: number }>;

    // Aggregate losses per user (loser_id)
    const lossesRows = db.prepare(`
      SELECT loser_id, COUNT(*) as losses, COALESCE(SUM(wager), 0) as lost_total
      FROM duel_history
      WHERE is_tie = 0
      GROUP BY loser_id
    `).all() as Array<{ loser_id: string; losses: number; lost_total: number }>;

    // Aggregate ties per user (cả winner_id và loser_id - cả 2 đều được tính)
    const tiesRows = db.prepare(`
    ...
    `).all() as Array<{ uid: string; ties: number }>;

    // Build a map by userId
    const playerMap = new Map<string, {
      wins: number; losses: number; ties: number;
      netEarned: number; lostTotal: number;
    }>();
    ...
    const allSorted = Array.from(playerMap.entries())
      ...
      .sort((a, b) => {
        ...
        if (b.netWinnings !== a.netWinnings) return b.netWinnings - a.netWinnings;
   ...
   */
  private reduceDurabilityAfterDuel(duel: DuelState): void {
    try {
   ...
   */
  private cleanExpiredDuels(): void {
    const now = Math.floor(Date.now() / 1000);
  ...
  private seasonSchedulerInterval: NodeJS.Timeout | null = null;
  private lastSeasonCheck: number = 0;

  /**
   * Khởi động scheduler kiểm tra mùa giải quyết đấu mỗi 60 giây
   */
  public startSeasonScheduler(): void {
    if (this.seasonSchedulerInterval) return;
   ...
   */
  private checkSeasonRotation(): void {
    const now = Math.floor(Date.now() / 1000);

    // Tránh kiểm tra quá thường xuyên (chỉ kiểm tra mỗi 30 giây)
    if (now - this.lastSeasonCheck < 30) return;
    this.lastSeasonCheck = now;

    // Lấy mùa hiện tại đang active
    const activeSeason = db.prepare(
      "SELECT * FROM duel_weekly_seasons WHERE status = 'active' ORDER BY id DESC LIMIT 1"
    ).get() as { id: number; season_number: number; week_start: number; week_end: number; status: string; rewards_given: number } | undefined;
   ...
   */
  private createNewSeason(now: number): void {
    const { weekStart, weekEnd } = this.getCurrentWeekBounds(now);

    // Đếm số mùa đã có
    const countResult = db.prepare("SELECT COUNT(*) as c FROM duel_weekly_seasons").get() as { c: number };
   ...
   */
  private finalizeSeason(season: { id: number; season_number: number; week_start: number; week_end: number },
    now: number): void {
    try {
      ...
      const finalizeTx = db.transaction((players: typeof topPlayers) => {
        for (let i = 0; i < players.length; i++) {
          ...
          `).run(
            ...
            prize.tuVi,
            prize.ngotinh,
            JSON.stringify(prize.items),
            now
          );

          // Trao thưởng trực tiếp
          const user = userRepository.get(player.userId);
          if (user) {
            // Linh Thạch
            userRepository.update(player.userId, {
            ...
            for (const item of prize.items) {
              inventoryRepository.addItem(player.userId, item.item_id, item.quantity);
            }
   ...
   */
  private getWeeklyTopPlayers(weekStart: number, weekEnd: number, limit: number): Array<{
    userId: string; name: string; wins: number; losses: number; netWinnings: number; totalMatches: number
  }> {
    // Thắng
    const winsRows = db.prepare(`
      SELECT winner_id, COUNT(*) as wins, COALESCE(SUM(winnings - wager), 0) as net_earned
      ...
      GROUP BY winner_id
    `).all(weekStart, weekEnd) as Array<{ winner_id: string; wins: number; net_earned: number }>;
    ...
    const lossesRows = db.prepare(`
      ...
      WHERE is_tie = 0 AND fought_at >= ? AND fought_at < ?
      GROUP BY loser_id
    `).all(weekStart, weekEnd) as Array<{ loser_id: string; losses: number; lost_total: number }>;

    // Build map
    const playerMap = new Map<string, { wins: number; losses: number; netEarned: number; lostTotal: number }>();
   ...
   */
  public getLastSeasonTop(): {
    season: { id: number; season_number: number; week_start: number; week_end: number } | null;
    top: Array<{
      rank: number;
      userId: string;
      userName: string;
      ...
      rewardItems: Array<{ item_id: string; quantity: number }>;
      claimed: boolean;
    }>;
  } {
    ...
    const topRecords = db.prepare(`
      ...
      WHERE season_id = ?
      ORDER BY rank ASC
    `).all(lastSeason.id) as any[];

    const top = topRecords.map((r: any) => ({
      rank: r.rank,
      userId: r.user_id,
   ...
   */
  public getCurrentSeason(): { season: { id: number; season_number: number; week_start: number; week_end: number } | null; hasStarted: boolean } {
   ...
   */
  private getCurrentWeekBounds(now: number): { weekStart: number; weekEnd: number } {
