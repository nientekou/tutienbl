...
export type EffectType = 'burn' | 'poison' | 'bleed' | 'curse' | 'stun' | 'freeze' | 'root' | 'sleep' | 'confusion' | 'silence' | 'blind' | 'weakness' | 'vulnerability' | 'haste' | 'shield';
...
export const EFFECT_DEFINITIONS: Record<EffectType, { name: string; description: string; maxStacks: number; defaultDuration: number; isDebuff: boolean }> = {
...
class CombatEffectService {
   ...
   */
  createEffect(type: EffectType, value: number, source: string, duration?: number): CombatEffect {
   ...
   */
  applyEffect(effects: CombatEffect[], newEffect: CombatEffect): CombatEffect[] {
   ...
   */
  processEffects(effects: CombatEffect[]): { damage: number; heal: number; log: string[] } {
    let totalDamage = 0;
    let totalHeal = 0;
    const log: string[] = [];
    ...
    for (const effect of effects) {
      if (effect.type === 'burn' || effect.type === 'poison' || effect.type === 'bleed' || effect.type === 'curse') {
        ...
        totalDamage += dmg;
        log.push(`${EFFECT_DEFINITIONS[effect.type].name}: -${dmg} HP`);
      }
      if (effect.type === 'haste') {
        log.push(`${EFFECT_DEFINITIONS[effect.type].name}: +${Math.round(effect.value * 100)}% Speed`);
      }
      if (effect.type === 'shield') {
        log.push(`${EFFECT_DEFINITIONS[effect.type].name}: Shield active`);
      }
    }
   ...
   */
  isCC(effects: CombatEffect[]): boolean {
   ...
   */
  isSilenced(effects: CombatEffect[]): boolean {
   ...
   */
  getEffectsDescription(effects: CombatEffect[]): string {
    if (effects.length === 0) return 'Không có hiệu ứng nào';
...
export const combatEffectService = new CombatEffectService();
