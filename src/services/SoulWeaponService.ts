...
class SoulWeaponService {
  ...
  private readonly WEAPON_FORMS: Record<string, { name: string; passive_desc: string; stat_bonus: string; value: number }> = {
  ...
  public canAwakenForm(userId: string): { eligible: boolean; reason: string } {
    const sw = soulWeaponRepository.getByUserId(userId);
    if (!sw) return { eligible: false, reason: 'Chưa có Pháp Bảo.' };
    if (sw.evolution_stage < 5) return { eligible: false, reason: `Cần stage 5 (hiện stage ${sw.evolution_stage})` };
    if (sw.awakened_form) return { eligible: false, reason: 'Đã biến hình rồi!' };
    return { eligible: true, reason: '' };
  }

  public awakenForm(userId: string): { success: boolean; message: string } {
    ...
    if (!form) return { success: false, message: '❌ Element không hợp lệ.' };
    ...
    db.prepare('UPDATE soul_weapons SET awakened_form = ? WHERE id = ?').run(form.name, sw.id);
  ...
  public getAwakenedForm(userId: string): { name: string; passive_desc: string } | null {
    const sw = soulWeaponRepository.getByUserId(userId);
    if (!sw || !sw.awakened_form) return null;
    const form = this.WEAPON_FORMS[sw.element];
