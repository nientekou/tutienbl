...
export class EnhanceService {
  ...
  public reforge(userId: string, inventoryId: number): { success: boolean; message: string } {
    const item = invRepo.get(inventoryId);
    if (!item || item.user_id !== userId) return { success: false, message: '❌ Vật phẩm không tồn tại.' };
    if (item.is_equipped !== 1) return { success: false, message: '❌ Chỉ có thể rèn lại trang bị đang đeo.' };

    // Check reforge count
    const customStats = item.custom_stats ? JSON.parse(item.custom_stats) : {};
    const reforgeCount = customStats._reforge_count || 0;
    if (reforgeCount >= 3) return { success: false, message: '❌ Đã rèn lại tối đa 3 lần.' };

    const user = userRepository.get(userId);
    if (!user) return { success: false, message: '❌ Chưa tạo nhân vật.' };
    if (user.coin_ha_pham < 1000) return { success: false, message: '❌ Cần 1000 LT để rèn lại.' };

    // Check material
    const matInv = db.prepare('SELECT id, quantity FROM inventories WHERE user_id = ? AND item_id = ?')
      .get(userId, 'material_mid_stone') as { id: number; quantity: number } | undefined;
    if (!matInv || matInv.quantity < 50) return { success: false, message: '❌ Cần 50 Nguyên Liệu Trung Phẩm.' };
    ...
    if (baseValue === 0) return { success: false, message: '❌ Vật phẩm không có chỉ số này.' };
    ...
    db.prepare('UPDATE inventories SET quantity = quantity - 50 WHERE id = ?').run(matInv.id);
    db.prepare('UPDATE inventories SET base_stats = ?, custom_stats = ? WHERE id = ?')
