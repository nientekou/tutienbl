...
import { CombatEngine, Combatant, PetCombatConfig, CombatResult } from './CombatEngine';
import { inventoryService, ActiveStats } from './InventoryService';
import { bloodlineService } from './BloodlineService';

export interface NineHeavensProgress {
  user_id: string;
...
class NineHeavensService {
  ...
  public readonly FLOORS: Record<number, FloorMonster> = {
    1: {
      name: 'Thạch Đầu Nhân',
   ...
   */
  public getProgress(userId: string): NineHeavensProgress {
    const weekKey = this.getWeekKey();
    let row = db.prepare('SELECT * FROM nine_heavens_progress WHERE user_id = ?').get(userId) as NineHeavensProgress | undefined;
    ...
    if (!row) {
      ...
      if (row.last_reset_week !== weekKey) {
        db.prepare('UPDATE nine_heavens_progress SET attempts_this_week = 0, last_reset_week = ? WHERE user_id = ?')
          .run(weekKey, userId);
  ...
  public buyExtraAttempt(userId: string): { success: boolean; message: string; progress?: NineHeavensProgress } {
    ...
    db.transaction(() => {
      ...
      db.prepare('UPDATE nine_heavens_progress SET attempts_this_week = attempts_this_week - 1 WHERE user_id = ?').run(userId);
    })();
   ...
   */
  public enterFloorChallenge(userId: string, forceBuy: boolean = false): { 
    success: boolean; 
    message: string; 
    requireBuy?: boolean; 
    ...
    if (isInfiniteMode) {
      ...
      if (progress.attempts_this_week >= 3 && forceBuy) {
        ...
        if (!buyRes.success) return { success: false, message: buyRes.message };
      }
      ...
      const petRaw = db.prepare('SELECT name, base_atk, mutations, skills FROM pets WHERE user_id = ? AND is_deployed = 1').get(userId) as any;
      let pet: PetCombatConfig | null = null;
      if (petRaw) {
      ...
      const combatResult = CombatEngine.run(playerCombatant, enemyCombatant, pet, 25, false, false);
      ...
      db.transaction(() => {
        ...
        if (combatResult.winner === 'player') {
          db.prepare('UPDATE nine_heavens_progress SET highest_floor = ? WHERE user_id = ?').run(nextFloor, userId);
    ...
    const avgLevelRow = db.prepare('SELECT AVG(level) as avg_level FROM users').get() as { avg_level: number } | undefined;
    ...
    const combatResult = CombatEngine.run(
      playerCombatant,
      enemyCombatant,
      pet ? { name: pet.name, atk: pet.atk } : null,
      maxRounds,
      false,
      isSurvival
    );
    ...
    db.transaction(() => {
      ...
      if (combatResult.winner === 'player') {
        ...
        let extraLoot = '';
        const statsUpdates: Partial<UserEntity> = {};
        if (nextFloor === 1) {
        ...
        if (nextFloor === 9) {
          const existingTitle = db.prepare('SELECT id FROM user_titles WHERE user_id = ? AND title = ?').get(userId, 'Thiên Trụ');
          if (!existingTitle) {
            db.prepare(`
          ...
          if (user.title === 'Tán Tu' || !user.title) {
            userRepository.update(userId, { title: 'Thiên Trụ' });
          }
        }

        // Cộng EXP cho Bản Mệnh Pháp Bảo
        let artifactMessage = '';
        const artifactRes = inventoryService.addArtifactExp(userId, 100 * nextFloor);
...
export const nineHeavensService = new NineHeavensService();
