...
class WorldBossReworkService {
  ...
  getBossDescription(bossId: string): string {
    ...
    const phase = BOSS_PHASES.find(p => p.phase === (info.phase || 1)) || BOSS_PHASES[0];
  ...
  private readonly phase5AttackCount = new Map<string, number>();

  public getPhaseV2Description(): string {
   ...
   */
  public applyPhaseMechanics(boss: any, hpPercent: number): { atkMult: number; defMult: number; healAmount: number; absorbPct: number; phase5Attacks: number } {
    const phase = BOSS_PHASES.find(p => p.phase === (boss.phase || 1)) || BOSS_PHASES[0];
    const result = {
  ...
  public resetPhase5Count(bossId: string): void {
    this.phase5AttackCount.delete(bossId);
  }

  public getPhase5Attacks(bossId: string): number {
