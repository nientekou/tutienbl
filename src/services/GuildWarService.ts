...
class GuildWarService {
  ...
  attack(warId: string, userId: string): { success: boolean; message: string; embedData?: any } {
    ...
    // Random damage
    const baseDamage = Math.round((activeStats.atk * 0.5 + activeStats.hp * 0.1) * levelBonus * luckBonus);
    ...
    return {
      success: true,
      message: `⚔️ **${user.name}** tung chiêu! ${critText}\nGây **${finalDamage}** sát thương lên **${targetName}**!\n\n` +
  ...
  private distributeRewards(warId: string, winnerSectId: number): void {
    ...
    for (const p of winnerParticipants) {
      ...
      const rewardCoin = Math.round(500 * share);
      const rewardExp = Math.round(1000 * share);
      const contrReward = Math.round(300 * share);
      ...
      db.transaction(() => {
        ...
        // Thưởng rương cho MVP
        if (share >= 0.4) {
          inventoryRepository.addItem(p.user_id, 'lucky_chest', 1);
        }
      })();
    ...
    if (sect) {
      const rewardResources = 2000 + war.current_round * 500;
