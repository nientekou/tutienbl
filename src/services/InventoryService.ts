...
export interface ActiveStats {
  ...
  rare_fire_passive?: string;
  rare_fire_value?: number;
  rare_beast_passive?: string;
  rare_beast_value?: number;
  mp_regen?: number;
}
...
export class InventoryService {
  ...
  public getActiveStats(userId: string): ActiveStats | null {
    ...
    try {
      ...
      if (daoBonuses.atk_bonus) multipliers.atk += daoBonuses.atk_bonus / 100;
      if (daoBonuses.def_bonus) multipliers.def += daoBonuses.def_bonus / 100;
      if (daoBonuses.hp_bonus) { stats.hp += daoBonuses.hp_bonus; }
      if (daoBonuses.crit_bonus) stats.crit += daoBonuses.crit_bonus;
      if (daoBonuses.dodge_bonus) stats.dodge += daoBonuses.dodge_bonus;
