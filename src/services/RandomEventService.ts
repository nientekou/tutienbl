...
// A-04: Random Event System

type EventType = 'lucky_find' | 'mysterious_npc' | 'hidden_treasure' | 'monster_surge' | 'weather_event';
...
class RandomEventService {
   ...
   */
  checkRandomEvent(userId: string, activityType: string): RandomEvent | null {
    this.initTable();

    // Filter events by activity type
    const relevantEvents = RANDOM_EVENTS.filter(e => {
      if (activityType === 'work' && (e.type === 'lucky_find' || e.type === 'monster_surge')) return true;
      if (activityType === 'explore' && (e.type === 'hidden_treasure' || e.type === 'mysterious_npc')) return true;
      if (activityType === 'combat' && (e.type === 'monster_surge' || e.type === 'weather_event')) return true;
      if (activityType === 'social' && e.type === 'mysterious_npc') return true;
      return false;
    });
    ...
    for (const event of relevantEvents) {
      if (Math.random() < event.chance) {
        // Record event
        db.prepare('INSERT INTO random_events (user_id, event_id, triggered_at) VALUES (?, ?, ?)')
   ...
   */
  applyEventEffect(userId: string, event: RandomEvent): { success: boolean; message: string } {
    ...
    switch (event.effect) {
      ...
      case 'exp_boost':
        userRepository.update(userId, { tu_vi: Math.min(user.tu_vi + 1000, user.exp_needed) });
      ...
      case 'mystery_chest':
        userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham + 2000, knb: user.knb + 5 });
   ...
   */
  getEventDescription(event: RandomEvent): string {
   ...
   */
  getRecentEvents(userId: string, limit: number = 5): { eventName: string; triggeredAt: number }[] {
    this.initTable();
...
export const randomEventService = new RandomEventService();
