...
class FishingService {
   ...
   */
  fish(userId: string, locationId: string = 'lake'): { success: boolean; message: string; fish?: FishDef; quality?: string } {
    this.initTable();
    ...
    let quality: string;
    if (qualityRoll < 0.10) quality = 'perfect';
    ...
    ).get(userId, caughtFish.id) as { id: number; quantity: number } | undefined;
    if (existingItem) {
      db.prepare('UPDATE inventories SET quantity = quantity + 1 WHERE id = ?').run(existingItem.id);
    } else {
      db.prepare(
        'INSERT INTO inventories (user_id, item_id, quantity, is_equipped) VALUES (?, ?, 1, 0)'
      ).run(userId, caughtFish.id);
    }
