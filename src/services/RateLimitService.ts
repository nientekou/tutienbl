...
class RateLimitService {
   ...
   */
  checkRateLimit(userId: string, action: string = 'default'): { allowed: boolean; retryAfter?: number } {
    ...
    if (entry.minuteStart < minuteStart) { entry.minuteCount = 0; entry.minuteStart = minuteStart; }
    if (entry.hourStart < hourStart) { entry.hourCount = 0; entry.hourStart = hourStart; }
    if (entry.dayStart < dayStart) { entry.dayCount = 0; entry.dayStart = dayStart; }
   ...
   */
  getRateLimitStats(userId: string): Record<string, { minute: number; hour: number; day: number }> {
    const stats: Record<string, { minute: number; hour: number; day: number }> = {};
