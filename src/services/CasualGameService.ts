...
class CasualGameService {
   ...
   */
  rollDice(userId: string, betAmount: number): { success: boolean; message: string } {
    const user = userRepository.get(userId);
    if (!user) return { success: false, message: '❌ Chưa tạo nhân vật!' };
    if (betAmount < 10) return { success: false, message: 'Tối thiểu cược **10** Linh Thạch!' };
    if (betAmount > 10000) return { success: false, message: 'Tối đa cược **10,000** Linh Thạch!' };
    if (user.coin_ha_pham < betAmount) return { success: false, message: `Không đủ Linh Thạch! (Cần ${betAmount}, có ${user.coin_ha_pham})` };
    ...
    userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham - betAmount + payout });
   ...
   */
  playCard(userId: string, betAmount: number): { success: boolean; message: string } {
    const user = userRepository.get(userId);
    if (!user) return { success: false, message: '❌ Chưa tạo nhân vật!' };
    if (betAmount < 10) return { success: false, message: 'Tối thiểu cược **10** Linh Thạch!' };
    if (betAmount > 10000) return { success: false, message: 'Tối đa cược **10,000** Linh Thạch!' };
    if (user.coin_ha_pham < betAmount) return { success: false, message: `Không đủ Linh Thạch!` };
   ...
   */
  fish(userId: string): { success: boolean; message: string } {
    const user = userRepository.get(userId);
    if (!user) return { success: false, message: '❌ Chưa tạo nhân vật!' };
    if (user.stamina < 10) return { success: false, message: `Không đủ Thể Lực! (Cần 10, có ${user.stamina})` };

    userRepository.update(userId, { stamina: user.stamina - 10 });
    ...
    userRepository.update(userId, { coin_ha_pham: user.coin_ha_pham + fish.value });
...
export const casualGameService = new CasualGameService();
