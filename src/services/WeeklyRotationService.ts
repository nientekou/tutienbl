...
class WeeklyRotationService {
   ...
   */
  getWeeklyRotation(): WeeklyRotation {
    this.initTable();
    const weekKey = this.getWeekKey();

    let rotation = db.prepare('SELECT * FROM weekly_rotations WHERE week_key = ?').get(weekKey) as any;
    if (!rotation) {
   ...
   */
  isWeeklyBonusActive(type: string, id: string): boolean {
    const rotation = this.getWeeklyRotation();
    switch (type) {
   ...
   */
  getMarketDiscount(category: string): number {
    const rotation = this.getWeeklyRotation();
...
export const weeklyRotationService = new WeeklyRotationService();
