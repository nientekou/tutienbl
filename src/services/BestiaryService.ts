// V13 E-02: Sách Yêu Khoa (Bestiary)
import db from '../database/database';

interface BestiaryEntry {
  enemy_id: string;
  enemy_name: string;
  zone: string;
  element: string;
  times_defeated: number;
  first_seen: number;
  last_seen: number;
}

class BestiaryService {
  private initTable(): void {
    db.exec(`
      CREATE TABLE IF NOT EXISTS bestiary (
        user_id TEXT NOT NULL,
        enemy_id TEXT NOT NULL,
        enemy_name TEXT NOT NULL,
        zone TEXT DEFAULT '',
        element TEXT DEFAULT '',
        times_defeated INTEGER DEFAULT 0,
        first_seen INTEGER NOT NULL,
        last_seen INTEGER NOT NULL,
        PRIMARY KEY (user_id, enemy_id)
      );
    `);
  }

  public recordKill(userId: string, enemyId: string, enemyName: string, zone: string, element: string): void {
    this.initTable();
    const now = Math.floor(Date.now() / 1000);
    db.prepare(`
      INSERT INTO bestiary (user_id, enemy_id, enemy_name, zone, element, times_defeated, first_seen, last_seen)
      VALUES (?, ?, ?, ?, ?, 1, ?, ?)
      ON CONFLICT(user_id, enemy_id) DO UPDATE SET
        times_defeated = times_defeated + 1,
        last_seen = excluded.last_seen
    `).run(userId, enemyId, enemyName, zone, element, now, now);
  }

  public getEntries(userId: string, zone?: string): BestiaryEntry[] {
    this.initTable();
    if (zone) {
      return db.prepare('SELECT * FROM bestiary WHERE user_id = ? AND zone = ? ORDER BY enemy_name')
        .all(userId, zone) as BestiaryEntry[];
    }
    return db.prepare('SELECT * FROM bestiary WHERE user_id = ? ORDER BY zone, enemy_name')
      .all(userId) as BestiaryEntry[];
  }

  public getZoneProgress(userId: string, zone: string): { defeated: number; total: number; completionRate: number } {
    this.initTable();
    const defeated = db.prepare('SELECT COUNT(*) as c FROM bestiary WHERE user_id = ? AND zone = ? AND times_defeated > 0')
      .get(userId, zone) as { c: number };
    // Total enemies in zone — approximate from all players' bestiary
    const total = db.prepare('SELECT COUNT(DISTINCT enemy_id) as c FROM bestiary WHERE zone = ?')
      .get(zone) as { c: number };
    return {
      defeated: defeated.c,
      total: total.c || 1,
      completionRate: total.c > 0 ? defeated.c / total.c : 0,
    };
  }

  public getCompletionReward(userId: string, zone: string): { completed: boolean; masterCompleted: boolean } {
    const progress = this.getZoneProgress(userId, zone);
    const entries = this.getEntries(userId, zone);
    const allDefeated = entries.length > 0 && entries.every(e => e.times_defeated > 0);
    const allMastered = entries.length > 0 && entries.every(e => e.times_defeated >= 10);
    return { completed: allDefeated, masterCompleted: allMastered };
  }

  public getDescription(userId: string): string {
    const entries = this.getEntries(userId);
    if (entries.length === 0) return '📖 **Sách Yêu Khoa** — Chưa ghi nhận enemy nào.';

    // Group by zone
    const byZone: Record<string, BestiaryEntry[]> = {};
    for (const e of entries) {
      const z = e.zone || 'Khác';
      if (!byZone[z]) byZone[z] = [];
      byZone[z].push(e);
    }

    let msg = `📖 **Sách Yêu Khoa** — ${entries.length} enemy đã gặp\n\n`;
    for (const [zone, zoneEntries] of Object.entries(byZone)) {
      const defeated = zoneEntries.filter(e => e.times_defeated > 0).length;
      msg += `**${zone}**: ${defeated}/${zoneEntries.length} ✅\n`;
    }
    return msg;
  }
}

export const bestiaryService = new BestiaryService();
