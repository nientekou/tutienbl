...
export interface QuestDefinition {
  ...
  minRealm?: number; // B2: Realm gate for quest availability
}
...
class DailyQuestService {
  ...
  getStreak(userId: string): StreakRow {
    const row = db.prepare('SELECT * FROM daily_quest_streaks WHERE user_id = ?').get(userId) as StreakRow | undefined;
    if (!row) {
  ...
  getOrAssignQuests(userId: string): (DailyQuestRow & { definition: QuestDefinition })[] {
    ...
    const availableQuests = QUEST_POOL.filter(q => !q.minRealm || playerRealm >= q.minRealm);
    ...
    const insertTx = db.transaction(() => {
      for (const quest of selected) {
        ...
        insertStmt.run(
          userId,
          quest.id,
          quest.required * reqMultiplier,
          Math.round(quest.rewardCoin * rewardMultiplier),
          Math.round(quest.rewardTuVi * rewardMultiplier),
          Math.round(quest.rewardNgotinh * rewardMultiplier),
          isElite,
          nowDay,
          expiresAt
        );
   ...
   */
  private areAllQuestsCompleted(userId: string): boolean {
    const nowDay = this.getTodayStartTimestamp();
   ...
   */
  private updateStreak(userId: string): { streak: number; isStreakReset: boolean; isWeekBonus: boolean } {
  ...
  getWeeklySummary(userId: string): string {
    ...
    `).all(userId, weekStartTs) as DailyQuestRow[];
