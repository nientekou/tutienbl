...
export class CultivationService {
    ...
    if (!isMajor) {
      ...
      if (typeof usePill === 'string' && usePill === 'bequan') {
        ...
        if (pill) {
          ...
          if (usePill === 'pill_break_minor_1') { pillBonus = 15; usedPillName = 'Tụ Khí Đan'; }
          else if (usePill === 'pill_break_minor_2') { pillBonus = 30; usedPillName = 'Bồi Nguyên Đan'; }
          else if (usePill === 'pill_break_minor_3') { pillBonus = 50; usedPillName = 'Tạo Hóa Đan'; }
        } else {
          return {
            success: false,
            isMajor: false,
      ...
      if (typeof usePill === 'string' && usePill === 'bequan') {
        ...
        if (breakPill) {
          ...
          inventoryRepository.removeItem(discordId, 'pill_break_1', 1);
        } else {
          return {
            success: false,
            isMajor: true,
  ...
  public awakenYCanh(discordId: string): { success: boolean; message: string; yCanhName?: string; newLevel?: number } {
    ...
    try {
      const yCanhMap: Record<string, number> = JSON.parse(user.y_canh || '{}');
