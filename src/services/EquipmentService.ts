...
export class EquipmentService {
  ...
  public autoEquip(userId: string): { success: boolean; message: string; equipped: string[] } {
    ...
    for (const slot of slots) {
      ...
      const gradeOrder: Record<string, number> = { f: 0, d: 1, c: 2, b: 3, a: 4, s: 5, ss: 6, sss: 7, ex: 8 };
      candidates.sort((a, b) => {
      ...
      if (currentInSlot) {
        inventoryRepository.updateEquipmentStatus(currentInSlot.id, 0, null);
      }

      // Equip best item
      inventoryRepository.updateEquipmentStatus(best.id, 1, slot);
  ...
  private getItemSlot(itemId: string): string {
    if (itemId.startsWith('weapon_') || itemId.startsWith('sword_') || itemId.startsWith('blade_') || itemId.startsWith('spear_') || itemId.startsWith('staff_')) return 'weapon';
    if (itemId.startsWith('armor_') || itemId.startsWith('robe_')) return 'armor';
    if (itemId.startsWith('ring_')) return 'ring';
    if (itemId.startsWith('necklace_') || itemId.startsWith('amulet_')) return 'necklace';
    if (itemId.startsWith('pendant_') || itemId.startsWith('boi_pham_')) return 'pendant';
    if (itemId.startsWith('mount_')) return 'mount';
    if (itemId.startsWith('treasure_')) return 'treasure';
    return 'unknown';
  }
}
