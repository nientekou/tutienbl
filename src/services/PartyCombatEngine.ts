...
import { checkPartyElementalCycle } from './PartyService';
...
export interface PartyBossConfig {
  name: string;
  hp: number;
  maxHp: number;
  atk: number;
...
export interface PartyCombatResult {
  victory: boolean;
  rounds: number;
 ...
 */
export class PartyCombatEngine {
  /**
   * Chạy trận đấu tổ đội 3v1 chống Boss
   */
  public static run(
    members: PartyMember[],
    boss: PartyBossConfig,
    maxRounds: number = 40
  ): PartyCombatResult {
    let bossHp = boss.hp;
    ...
    const sortedMembers = [...aliveMembers].sort((a, b) => 
      (b.combatant.speed || 100) - (a.combatant.speed || 100)
    );
    ...
    const cycleInfo = checkPartyElementalCycle(userIds);
    ...
    const rewards = {
      exp: baseExp,
      coins: baseCoins,
      loots: [] as Array<{ id: string; name: string; quantity: number }>
    };
  ...
  private static executeBossTurn(
    ...
    const aliveMembers = members.filter(m => m.isAlive);
    if (aliveMembers.length === 0) return bossHp;
   ...
   */
  public static distributeRewards(
    result: PartyCombatResult,
    bossLevel: number
  ): Map<string, { exp: number; coins: number }> {
    const totalDamage = Array.from(result.damageByPlayer.values()).reduce((a, b) => a + b, 0) || 1;
    const rewards = new Map<string, { exp: number; coins: number }>();
...
export default PartyCombatEngine;
