...
class SectWarService {
  ...
  public attack(userId: string, targetUserId: string): { success: boolean; message: string; damage?: number } {
    ...
    if (!user || !target) return { success: false, message: 'Đạo hữu chưa khởi tạo nhân vật.' };
    if (!user.sect_id) return { success: false, message: 'Đạo hữu chưa gia nhập Tông Môn!' };
   ...
   */
  awardWarPoints(userId: string, won: boolean): { points: number; streakBonus: number } {
   ...
   */
  getSeasonRewards(): { rank: string; reward: string }[] {
    return [
   ...
   */
  getSeasonInfo(): { seasonNumber: number; status: string; daysLeft: number } {
    const season = this.getOrCreateSeason();
    const now = Math.floor(Date.now() / 1000);
   ...
   */
  assignWarRole(userId: string, role: 'attacker' | 'defender' | 'support'): { success: boolean; message: string } {
    const user = userRepository.get(userId);
    if (!user || !user.sect_id) return { success: false, message: '❌ Chưa gia nhập Tông Môn!' };

    const roleBonuses: Record<string, { stat: string; value: number }> = {
    ...
    try {
      ...
      userRepository.update(userId, { y_canh: JSON.stringify({ war_role: role }) });
    }
   ...
   */
  getWarRoleBonus(userId: string): { role: string; bonus: number } {
    const user = userRepository.get(userId);
    if (!user) return { role: 'none', bonus: 0 };

    try {
      const yCanh = JSON.parse(user.y_canh || '{}');
      const role = yCanh.war_role || 'none';
      const bonuses: Record<string, number> = { attacker: 0.20, defender: 0.20, support: 0.20 };
   ...
   */
  captureSiegePoint(userId: string, pointId: string): { success: boolean; message: string } {
    const user = userRepository.get(userId);
    if (!user || !user.sect_id) return { success: false, message: '❌ Chưa gia nhập Tông Môn!' };
   ...
   */
  getSiegeStatus(): { points: { id: string; sectId: number; sectName: string; defensePower: number }[] } {
    try {
      const points = db.prepare('SELECT * FROM siege_points').all() as any[];
      return {
        points: points.map(p => {
          const sect = db.prepare('SELECT name FROM sects WHERE id = ?').get(p.sect_id) as any;
          return {
            id: p.point_id,
            sectId: p.sect_id,
   ...
   * B-01: Get siege maps
   */
  getSiegeMaps(): { id: string; name: string; description: string; capturePoints: number }[] {
    return [
   ...
   */
  getSiegeSpectating(siegeId: string): { viewers: number; topDamage: { userId: string; name: string; damage: number }[] } {
    const viewers = db.prepare('SELECT COUNT(*) as c FROM arena_spectators WHERE match_id = ?').get(siegeId) as { c: number };
   ...
   */
  getSiegeHistory(limit: number = 10): { date: string; winner: string; loser: string; score: number }[] {
    return db.prepare(`
      SELECT
