...
// E-04: Secret Skills System

interface SecretSkillDef {
  id: string;
  name: string;
  description: string;
  effect: string;
...
const SECRET_SKILLS: SecretSkillDef[] = [
...
class SecretSkillService {
   ...
   */
  isUnlocked(userId: string, skillId: string): boolean {
    this.initTable();
    const row = db.prepare('SELECT * FROM user_secret_skills WHERE user_id = ? AND skill_id = ?')
      .get(userId, skillId) as UserSecretSkill | undefined;
   ...
   */
  unlockSkill(userId: string, skillId: string): { success: boolean; message: string } {
    this.initTable();
    ...
    const existing = db.prepare('SELECT * FROM user_secret_skills WHERE user_id = ? AND skill_id = ?')
      .get(userId, skillId) as UserSecretSkill | undefined;

    if (existing?.unlocked === true) return { success: false, message: 'Kỹ năng này đã được mở khoá!' };

    db.prepare('INSERT OR REPLACE INTO user_secret_skills (user_id, skill_id, unlocked) VALUES (?, ?, 1)')
      .run(userId, skillId);
   ...
   */
  useSkill(userId: string, skillId: string): { success: boolean; message: string; effect: string } {
    this.initTable();
    const skill = SECRET_SKILLS.find(s => s.id === skillId);
    if (!skill) return { success: false, message: 'Kỹ năng không tồn tại', effect: '' };
    ...
    // Check cooldown
    const row = db.prepare('SELECT * FROM user_secret_skills WHERE user_id = ? AND skill_id = ?')
      .get(userId, skillId) as UserSecretSkill;
    ...
    // Use skill
    userRepository.update(userId, { mp: (user.mp || 0) - skill.mpCost });
   ...
   */
  getUserSkills(userId: string): { skill: SecretSkillDef; unlocked: boolean; onCooldown: boolean }[] {
    this.initTable();
    const now = Math.floor(Date.now() / 1000);

    return SECRET_SKILLS.map(skill => {
      const row = db.prepare('SELECT * FROM user_secret_skills WHERE user_id = ? AND skill_id = ?')
        .get(userId, skill.id) as UserSecretSkill | undefined;
   ...
   */
  getSecretSkillsDescription(userId: string): string {
...
export const secretSkillService = new SecretSkillService();
