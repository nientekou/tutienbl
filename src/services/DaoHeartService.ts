...
interface DaoPath {
  ...
  nodes: { name: string; description: string; cost: number; stat: string; value: number }[];
}

const DAO_PATHS: Record<string, DaoPath> = {
  chinh_dao: {
    id: 'chinh_dao',
    name: 'Chính Đạo',
...
class DaoHeartService {
  ...
  public getPath(userId: string): string | null {
    this.initTable();
    const row = db.prepare('SELECT path FROM dao_heart WHERE user_id = ?').get(userId) as { path: string } | undefined;
    return row?.path || null;
  }

  public getDaoPoints(userId: string): number {
    this.initTable();
    const row = db.prepare('SELECT dao_points FROM dao_heart WHERE user_id = ?').get(userId) as { dao_points: number } | undefined;
    return row?.dao_points || 0;
  }

  public choosePath(userId: string, pathId: string): { success: boolean; message: string } {
    this.initTable();
    if (!DAO_PATHS[pathId]) return { success: false, message: '❌ Đạo tâm không hợp lệ.' };
    const existing = this.getPath(userId);
    if (existing) return { success: false, message: '❌ Đã chọn đạo tâm rồi! Không thể thay đổi.' };

    db.prepare('INSERT INTO dao_heart (user_id, path, dao_points, unlocked_nodes) VALUES (?, ?, 0, \'[]\')')
      .run(userId, pathId);
  ...
  public addDaoPoints(userId: string, amount: number): void {
    this.initTable();
    db.prepare('INSERT INTO dao_heart (user_id, path, dao_points, unlocked_nodes) VALUES (?, NULL, ?, \'[]\') ON CONFLICT(user_id) DO UPDATE SET dao_points = dao_points + ?')
      .run(userId, amount, amount);
  }

  public unlockNode(userId: string, nodeIndex: number): { success: boolean; message: string } {
    this.initTable();
    const pathId = this.getPath(userId);
    if (!pathId) return { success: false, message: '❌ Chưa chọn đạo tâm.' };

    const path = DAO_PATHS[pathId];
    const node = path.nodes[nodeIndex];
    if (!node) return { success: false, message: '❌ Node không hợp lệ.' };

    const row = db.prepare('SELECT unlocked_nodes FROM dao_heart WHERE user_id = ?').get(userId) as { unlocked_nodes: string } | undefined;
    const unlocked: number[] = JSON.parse(row?.unlocked_nodes || '[]');
    if (unlocked.includes(nodeIndex)) return { success: false, message: '❌ Đã mở node này.' };
    if (nodeIndex > 0 && !unlocked.includes(nodeIndex - 1)) return { success: false, message: `❌ Cần mở node ${nodeIndex} trước.` };

    const points = this.getDaoPoints(userId);
    if (points < node.cost) return { success: false, message: `❌ Cần ${node.cost} Đạo Điểm (hiện có: ${points}).` };

    unlocked.push(nodeIndex);
    db.prepare('UPDATE dao_heart SET dao_points = dao_points - ?, unlocked_nodes = ? WHERE user_id = ?')
      .run(node.cost, JSON.stringify(unlocked), userId);

    return { success: true, message: `✅ Đã mở **${node.name}**! ${node.description}` };
  }

  public getUnlockedNodes(userId: string): number[] {
    this.initTable();
    const row = db.prepare('SELECT unlocked_nodes FROM dao_heart WHERE user_id = ?').get(userId) as { unlocked_nodes: string } | undefined;
  ...
  public getDescription(userId: string): string {
    const pathId = this.getPath(userId);
    if (!pathId) return '📭 Chưa chọn đạo tâm. Dùng `/daotam chon <chinh_dao|ma_dao|trung_dao>`';
...
export const daoHeartService = new DaoHeartService();
