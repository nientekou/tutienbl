...
try {
  ...
  db.prepare('DELETE FROM boss_announcements WHERE guild_id = ?').run(guildId);
  db.prepare('DELETE FROM users WHERE discord_id = ?').run(userId);
  db.prepare('DELETE FROM world_boss_contributions WHERE boss_id = ?').run('world_boss_current');
  ...
  db.prepare('INSERT OR REPLACE INTO boss_announcements (guild_id, channel_id, message_id) VALUES (?, ?, ?)')
    .run(guildId, 'ch_boss_channel', 'msg_888888');

  const ann = db.prepare('SELECT * FROM boss_announcements WHERE guild_id = ?').get(guildId) as any;
  ...
  const contrib = db.prepare("SELECT * FROM world_boss_contributions WHERE user_id = ?").get(userId) as any;
  ...
  const finalBoss = db.prepare("SELECT * FROM world_boss WHERE id = 'world_boss_current'").get() as any;
  ...
  rewardsLogs.forEach((log: string) => console.log(`  └ ${log}`));
