...
export default class ReadyEvent extends Event<'ready'> {
  ...
  public async execute(client: TuTienClient): Promise<void> {
    ...
    try {
      ...
      table.push(['World Event Scheduler', chalk.green('✔ Running')]);
    } catch (error) {
      table.push(['World Event Scheduler', chalk.red('❌ LỖI')]);
    ...
    try {
      ...
      table.push(['Elite Hunt Scheduler', chalk.green('✔ Running')]);
    } catch (error) {
      table.push(['Elite Hunt Scheduler', chalk.red('❌ LỖI')]);
      console.error(chalk.red('[System] Lỗi Elite Hunt Scheduler:'), error);
    }
