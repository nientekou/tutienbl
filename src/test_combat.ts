...
try {
  ...
  db.prepare('DELETE FROM dungeon_cooldowns WHERE user_id IN (?, ?)').run(userIdA, userIdB);
  db.prepare('DELETE FROM world_boss_contributions WHERE user_id IN (?, ?)').run(userIdA, userIdB);
  ...
  userRepository.update(userIdB, { level: 40, coin_ha_pham: 500 });
  ...
  const res1 = combatService.challengeDungeon(userIdA, 'dungeon_luyen_khi_1');
  ...
  const res2 = combatService.challengeDungeon(userIdA, 'dungeon_truc_co_1');
  ...
  const resB1 = combatService.challengeDungeon(userIdB, 'dungeon_truc_co_1');
  ...
  const bossStatusBefore = combatService.getCurrentBoss();
  ...
  const wbRes1 = combatService.challengeWorldBoss(userIdB, false);
  console.log(`- Kết quả: ${wbRes1.message}`);
  console.log(`- Sát thương gây ra: ${wbRes1.damageDealt}`);
  
  const bossStatusAfter1 = combatService.getCurrentBoss();
  ...
  const wbRes2 = combatService.challengeWorldBoss(userIdB, false);
  ...
  db.prepare("UPDATE world_boss SET hp = 100 WHERE id = 'world_boss_current'").run();
  ...
  db.prepare("UPDATE world_boss_contributions SET last_attack_at = 0 WHERE user_id = ?").run(userIdB);

  const wbKill = combatService.challengeWorldBoss(userIdB, false);
  ...
  const bossStatusAfterDefeat = combatService.getCurrentBoss();
  ...
  db.prepare('DELETE FROM dungeon_cooldowns WHERE user_id IN (?, ?)').run(userIdA, userIdB);
  db.prepare('DELETE FROM world_boss_contributions WHERE user_id IN (?, ?)').run(userIdA, userIdB);
  db.prepare('DELETE FROM pets WHERE user_id IN (?, ?)').run(userIdA, userIdB);
